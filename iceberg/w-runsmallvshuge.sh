...
if [ -z ${SGE_TASK_ID+x} ] || [ "${SGE_TASK_ID}" == "undefined" ];then
	if [ -z ${STATECHUM_COUNT+x} ];then
		...
		sh ./runexperiment.sh -Xmx16000m ${EXPERIMENT} COLLECT_RESULTS
	else
		sh ./runexperiment.sh -Xmx4000m ${EXPERIMENT} COUNT_TASKS 100
	fi
else
# if task id is not "undefined", it means we are running an array task
	sh ./runexperiment.sh -Xmx16000m ${EXPERIMENT} RUN_TASK $SGE_TASK_ID
fi
