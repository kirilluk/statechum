...
public class TestGDExistingGraphsUsingTestSet {
	...
	public static Collection<Object[]> data() 
	{
		...
		for(int fileNum = 0;fileNum < files.length;++fileNum)
		{
			...
			boolean fallback = TestGDExistingGraphs.detectFallbackToInitialPair(fileA, null, fileB, null);
				...
				for(double ratio:new double[]{0.3,0.9})
					for(int pairs:new int[]{0})
						result.add(new Object[]{
								threadNo, pairs,ratio,fileA,fileB});
	...
	public TestGDExistingGraphsUsingTestSet(int th, int pairs, double ratio, File fileA, File fileB)
	{
	...
	public final void runPatch(File fileA, File fileB)
	{
		try
		{
			LearnerGraph grA = new LearnerGraph(config);AbstractPersistence.loadGraph(fileA, grA, converter);
            TestGDExistingGraphs.addColourAndIncompatiblesRandomly(grA, new Random(0));
			LearnerGraph grB = new LearnerGraph(config);AbstractPersistence.loadGraph(fileB, grB, converter);
            TestGDExistingGraphs.addColourAndIncompatiblesRandomly(grB, new Random(1));
			GD<CmpVertex,CmpVertex,LearnerGraphCachedData,LearnerGraphCachedData> gd = new GD<>();
			LearnerGraph graph = new LearnerGraph(config);AbstractPersistence.loadGraph(fileA, graph, converter);
            TestGDExistingGraphs.addColourAndIncompatiblesRandomly(graph, new Random(0));
