...
parseMap_test_() ->
	...
	{inparallel,
	[
			fun() -> testsynapse:useworker(fun(Pid,Ref) -> Pid!{Ref,testMapParsing,a},receive {Ref,failure,Text} -> ?assertEqual(true,testsynapse:containsCast(Text,"Atom "," com.ericsson.otp.erlang.OtpErlangList")) end end) end,
			fun() -> testsynapse:useworker(fun(Pid,Ref) -> Pid!{Ref,testMapParsing,[{a,b},a]},receive {Ref,failure,Text} -> ?assertEqual(true,testsynapse:containsCast(Text,"Atom "," com.ericsson.otp.erlang.OtpErlangTuple")) end end) end,
			...
			fun() -> testsynapse:useworker(fun(Pid,Ref) -> Pid!{Ref,testMapParsing,[{a,b},{"b",a}]},receive {Ref,failure,Text} -> ?assertEqual(true,testsynapse:containsCast(Text,"String "," com.ericsson.otp.erlang.OtpErlangAtom")) end end) end,
			fun() -> testsynapse:useworker(fun(Pid,Ref) -> Pid!{Ref,testMapParsing,[{a,b},{b,"a"}]},receive {Ref,failure,Text} -> ?assertEqual(true,testsynapse:containsCast(Text,"String "," com.ericsson.otp.erlang.OtpErlangAtom")) end end) end,
...
diffLoad_test_()->
	...
	[
			...
			#statemachinedifference{
				...
				name_mapping=[],
				initial_state=a}
			},receive {Ref,failure,Text} -> ?assertEqual(true,testsynapse:containsCast(Text,"Atom "," com.ericsson.otp.erlang.OtpErlangList")),ok end end) end,
