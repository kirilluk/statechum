...
public class SootCallGraphManager extends JFrame {
	...
	public void init() {
		...
		dynamicClasses
				.add("CH.ifa.draw.util.collections.jdk12.CollectionsFactoryJDK12");
	...
	private void selectPackageRoot() {
		...
		if (choice == JFileChooser.APPROVE_OPTION) {
			...
			Scene.v().setSootClassPath(
					sootClassPath + System.getProperty("path.separator")
							+ packageRoot.getPath());
	...
	private void selectMainClass() {
		String name = (String) JOptionPane.showInputDialog(this,
				"Enter name of main class", "Customized Dialog",
	...
	private void populateClasses(File[] analysisFiles) {
		...
		for (int i = 0; i < analysisFiles.length; i++) {
			...
			packagePath = f.getPath().toString().substring(
					rootPath.length() + 1);
			packagePath = packagePath.replace(File.separatorChar, '.');
			fileName = (packagePath.substring(0, packagePath.length() - 6));
			if (!(packagePath.substring(packagePath.length() - 6, packagePath
					.length()).equals(".class")))
				continue;
			try {
