...
public class ScoreMatrixOutput {
	...
	public static void main(String[] args){
		...
		Stack<PairScore> pairScores = gr.pairscores.chooseStatePairs(0,10,1,null,LearnerGraphND.ignoreRejectStates);
		ArrayList vertexList = new ArrayList();
		vertexList.addAll(dsg.getVertices());
		writeMatrix(pairScores,vertexList,vertexList);
	}


	public static void writeMatrix(List<PairScore> ps, List<StringVertex> stateLabelsA, List<StringVertex> stateLabelsB){
		String fileRef = statechum.GlobalConfiguration.getConfiguration().getProperty(statechum.GlobalConfiguration.G_PROPERTIES.TEMP)+File.separator+"scoreMatrixOutput.csv";
