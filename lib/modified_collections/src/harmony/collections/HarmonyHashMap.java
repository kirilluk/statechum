 ...
 */
public class HarmonyHashMap<K, V> extends HarmonyAbstractMap<K, V> implements Map<K, V>,
    ...
    private static class AbstractMapIterator<K, V>  {
        ...
        final HarmonyHashMap<K, V> associatedMap;

        AbstractMapIterator(HarmonyHashMap<K, V> hm) {
            associatedMap = hm;
    ...
    private static class EntryIterator <K, V> extends AbstractMapIterator<K, V> implements Iterator<Map.Entry<K, V>> {
        ...
        EntryIterator (HarmonyHashMap<K, V> map) {
            super(map);
        }
    ...
    private static class KeyIterator <K, V> extends AbstractMapIterator<K, V> implements Iterator<K> {
        ...
        KeyIterator (HarmonyHashMap<K, V> map) {
            super(map);
        }
    ...
    private static class ValueIterator <K, V> extends AbstractMapIterator<K, V> implements Iterator<V> {
        ...
        ValueIterator (HarmonyHashMap<K, V> map) {
            super(map);
        }
    ...
    static class HashMapEntrySet<KT, VT> extends AbstractSet<Map.Entry<KT, VT>> {
        private final HarmonyHashMap<KT, VT> associatedMap;

        public HashMapEntrySet(HarmonyHashMap<KT, VT> hm) {
            associatedMap = hm;
        }

        HarmonyHashMap<KT, VT> HarmonyHashMap() {
     ...
     */
    public HarmonyHashMap(int capacity) {
     ...
     */
    public HarmonyHashMap(int capacity, float loadFactor) {
     ...
     *            the mappings to add.
     */
    public HarmonyHashMap(Map<? extends K, ? extends V> map) {
    ...
    public Object clone() {
        try {
            HarmonyHashMap<K, V> map = (HarmonyHashMap<K, V>) super.clone();
            map.elementCount = 0;
    ...
    public Set<K> keySet() {
        if (keySet == null) {
            keySet = new AbstractSet<K>() {
                ...
                @Override
                public int size() {
                    return HarmonyHashMap.this.size();
                }

                @Override
                public void clear() {
                    HarmonyHashMap.this.clear();
                }

                @Override
                public boolean remove(Object key) {
                    Entry<K, V> entry = HarmonyHashMap.this.removeEntry(key);
                    return entry != null;
                }

                @Override
                public Iterator<K> iterator() {
                    return new KeyIterator<>(HarmonyHashMap.this);
                }
            };
        }
        return keySet;
        ...
        if (valuesCollection == null) {
            valuesCollection = new AbstractCollection<V>() {
                ...
                public Iterator<V> iterator() {
                    return new ValueIterator<>(HarmonyHashMap.this);
                }
            };
        }
