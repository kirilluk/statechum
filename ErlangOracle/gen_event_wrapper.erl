...
call_trace({Module, Pid}, [{call, T} | Trace], OpProc,Delay) ->
    OP = gen_event:call(Module, T, Delay),
    OpProc ! {self(), output, {call, T, OP}},
    call_trace({Module, Pid}, Trace, OpProc,Delay);
call_trace({Module, Pid}, [{call, T, OP} | Trace], OpProc,Delay) ->
    ThisOP = gen_event:call(Module, T, Delay),
    if (ThisOP =/= OP) ->
