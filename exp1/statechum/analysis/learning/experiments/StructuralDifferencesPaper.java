...
public class StructuralDifferencesPaper {
	...
	public static void dumpEquationsAndResults(LearnerGraph subject, GDLearnerGraph gdLearner){
		final int [] incompatiblePairs = new int[gdLearner.getPairNumber()];
		...
		LSolver solver = gdLearner.buildMatrix_internal(incompatiblePairs, pairsNumber, 1,null);
		double[] results = gdLearner.computeStateCompatibility(1, DDRH_default.class);
		System.out.println(gdLearner.dumpEquations(solver, incompatiblePairs, null).toString());	
		dumpResults(subject,gdLearner,results);
	}
	
	public static void dumpResults(LearnerGraph g, GDLearnerGraph ndGraph, double[] results){
		Iterator<CmpVertex> firstIteration = g.getTransitionMatrix().keySet().iterator();
		while(firstIteration.hasNext()){
			CmpVertex from = firstIteration.next();
			Iterator<CmpVertex> secondIteration = g.getTransitionMatrix().keySet().iterator();
			while(secondIteration.hasNext()){
				CmpVertex to = secondIteration.next();
				int pairNr = ndGraph.vertexToIntNR(from, to);
				System.out.println(from.getID()+", "+to.getID()+" => "+results[pairNr]);
			}
		}
	}
