...
public class TestErlangModule 
{
	...
	public static void beforeClass()
	{
		...
		for(String erlSource:List.of("locker/locker.erl","WibbleMonster/wibble.erl","exporter/exporter.erl","frequency/frequencyBroken.erl","frequency/frequencyFixed.erl","frequency/frequency.erl")) {
	...
	{
		statechum.Helper.checkForCorrectException(() -> ErlangModule.setupErlangConfiguration(config,new File(ErlangExamples,"WibbleMonster/wibble.erla")),IllegalArgumentException.class,"Invalid module");
	}
	...
	public void testRunParserFailure2() throws IOException
	{
		...
		else
		{
			...
			statechum.Helper.checkForCorrectException(() -> ErlangModule.loadModule(configWibble,true),ErlangThrownException.class,"Invalid file name");
		}
	}
    ...
    public void testTyperUnreadableBeamFile()
    {
		...
		Helper.checkForCorrectException(() -> runner.call(otpArgs,"Could not run dialyzer"),ErlangThrownException.class,"Failed to obtain file info");
    }
    ...
    public void testTyperWithInvalidPLTerror() throws IOException
    {
		...
		Helper.checkForCorrectException(() -> runOnlyTyperAsAProcessInsideErlang(file),ErlangThrownException.class,"has thrown");
		//noinspection ResultOfMethodCallIgnored
    ...
    public void testAttemptTracesNotInAlphabet()
    {
		...
		statechum.Helper.checkForCorrectException(() -> learner.askErlang(Arrays.asList(initLabel,labelLock,labelInvalidRead)),IllegalArgumentException.class,"does not belong");
    }
    ...
    public void testInvalidAttribute2() throws IOException
    {
		Writer wr = new FileWriter(erlangFile);wr.write("-module(testFile).\n-behaviour(56)."+stdFunctions);wr.close();
   		ErlangModule.setupErlangConfiguration(config,new File(erlangFile));
		checkForCorrectExceptionAnyOf(() -> ErlangModule.loadModule(config),IllegalArgumentException.class,List.of("\"8\" is of the wrong kind","atom_to_list,\"8\""));// 56 is interpreted as a string "8"
   }
    @Test
    public void testInvalidAttribute3() throws IOException
    {
		Writer wr = new FileWriter(erlangFile);wr.write("-module(testFile).\n-behaviour(565)."+stdFunctions);wr.close();
   		ErlangModule.setupErlangConfiguration(config,new File(erlangFile));
		checkForCorrectExceptionAnyOf(() -> ErlangModule.loadModule(config),IllegalArgumentException.class,List.of("565 is of the wrong type", "atom_to_list,[565]"));// Erlang 24 compiler throws an internal error on such an input file so instead of a nice error message we are looking up a chunk of text in the error trace.
   }
    ...
    public void testInvalidAttribute4() throws IOException
    {
		Writer wr = new FileWriter(erlangFile);wr.write("-module(testFile).\n-behaviour(\"junk\")."+stdFunctions);wr.close();
   		ErlangModule.setupErlangConfiguration(config,new File(erlangFile));
		checkForCorrectExceptionAnyOf(() -> ErlangModule.loadModule(config),IllegalArgumentException.class,List.of("\"junk\" is of the wrong kind","atom_to_list,[\"junk\"]"));
   }
