...
public class RPNIBlueFringeLearnerTestComponentOpt extends
	...
	protected void update(StatePair pair) {
		...
		pair.getR().setUserDatum(JUConstants.HIGHLIGHT, pair, UserData.SHARED);// since
																				// this
																				// copy
																				// of
																				// the
	...
	protected int counterAccepted = 0, counterRejected = 0,
			counterRestarted = 0, counterEmptyQuestions = 0;

	/**
	 * Takes the candidates for merging and computes the number of times
	 * different scores are encountered.
	 */
	public static void populateScores(Collection<PairScore> data,
			Map<Integer, AtomicInteger> histogram) {
		for (PairScore pair : data) {
			int pairScore = pair.getScore();
			AtomicInteger count = histogram.get(pairScore);
			if (count == null) {
				count = new AtomicInteger();
				histogram.put(pairScore, count);
			}
	...
	/**
	 * Takes the candidates for merging and computes the number of times
	 * different scores (increments of 10) are encountered.
	 */
	public static void populateHistogram(Collection<PairScore> data,
			Map<Integer, AtomicInteger> histogram) {
		for (PairScore pair : data) {
			int pairScore = pair.getScore() >= 200 ? pair.getScore()
					- pair.getScore() % 100 : pair.getScore() >= 10 ? pair
					.getScore()
					- pair.getScore() % 10 : pair.getScore() > 0 ? 1 : 0;
			AtomicInteger count = histogram.get(pairScore);
			if (count == null) {
				count = new AtomicInteger();
				histogram.put(pairScore, count);
	...
	public static String HistogramToString(
			Map<Integer, AtomicInteger> histogram, String Name) {
		final String FS = ",";
		String result = "\n" + Name;
	...
	public static String HistogramToSeries(
			Map<Integer, AtomicInteger> histogram, String Name) {
		final String FS = ",";
		String result = "\n" + Name;
		Map<Integer, AtomicInteger> tmp = new TreeMap<Integer, AtomicInteger>();
	...
	public static String pairScoresAndIterations(Map<PairScore, Integer> map,
			String name) {
		final String FS = ",";
		String result = "\n" + name + "-score" + FS;
	...
	public void init(Collection<List<String>> plus,
	...
	protected static void dumpSets(String output,
			Collection<List<String>> sPlus, Collection<List<String>> sMinus) {
		try {
