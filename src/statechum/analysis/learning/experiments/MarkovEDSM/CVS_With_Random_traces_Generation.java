			...
			for(final int preset: new int[]{0})//0,1,2})
			{
				...
				parExp.setExperimentID(traceQuantity,traceLengthMultiplierMax, alphabetMultiplier);
					...
					{// by the time we are here, experiments for the current number of states have completed, hence record the outcomes.
						final SquareBagPlot gr_StructuralDiff = new SquareBagPlot("Structural score, HV","Structural Score, EDSM-Markov learner",new File(outPathPrefix+preset+"_"+states+"_trace_structuraldiff.pdf"),0,1,true);
						final SquareBagPlot gr_BCR = new SquareBagPlot("BCR, HV","BCR, EDSM-Markov learner",new File(outPathPrefix+preset+"_"+states+"_trace_bcr.pdf"),0.5,1,true);
