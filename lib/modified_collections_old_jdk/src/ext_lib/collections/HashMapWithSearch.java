...
public class HashMapWithSearch<I, K extends I,V> implements MapWithSearch<I,K,V>
{
     ...
     *
     * @param  initialCapacityArg the initial capacity
     ...
     */
    void transfer(Entry<I,K,V>[] newTable, boolean rehash) {
        int newCapacity = newTable.length;
        for (Entry<I,K,V> e = header.after; e != header; e = e.after) {
            if (rehash)
	...
	public V get(Object key) {
        Entry<I,K,V> e = getEntry(key);
        if (e == null)
            return null;
        e.recordAccess(HashMapWithSearch.this);
        return e.value;
    }

	@Override
	public K findKey(I id) {
		Entry<I,K,V> e = getEntry(id);
        if (e == null)
            return null;
        e.recordAccess(HashMapWithSearch.this);
        return e.key;
     ...
     */
    public static class Entry<I,K extends I,V> implements Map.Entry<K, V>
    {
        final K key;
        V value;
        Entry<I,K,V> next;
         ...
         * Creates new entry.
         */
        Entry(int h, K k, V v, Entry<I,K,V> n) {
            value = v;
            next = n;
            key = k;
            hash = h;
         ...
         */
        void addBefore(Entry<I,K,V> existingEntry) {
            after  = existingEntry;
         ...
         */
        void recordAccess(@SuppressWarnings("unused") HashMapWithSearch<I,K,V> m) {
        }

        void recordRemoval(@SuppressWarnings("unused") HashMapWithSearch<I,K,V> m) {
            remove();
        }
    }    
    ...
    {
        Entry<I,K,V> nextEntry    = header.after;
    ...
    abstract class HashMapWithSearchOrderedIterator<T> extends HashMapWithSearchAbstractIterator<T> 
    {
    	...
    	{
            TreeSet<K> localKeySet = new TreeSet<>(HashMapWithSearch.this.keySet());
            keyIterator = localKeySet.iterator();
    	}
    ...
    /**
     * This differs from LinkedHashMap's addEntry in that it doesn't resize the
     * table or remove the eldest entry.
     */
    void createEntry(int hash, K key, V value, int bucketIndex) {
        Entry<I,K,V> old = table[bucketIndex];
        Entry<I,K,V> e = new Entry<>(hash, key, value, old);
        table[bucketIndex] = e;
        e.addBefore(header);
        size++;
    }
...
/*
     ...
     * for the key.
     */
    final Entry<I,K,V> getEntry(Object key) {
        int hash = (key == null) ? 0 : hash(key);
        for (Entry<I,K,V> e = table[indexFor(hash, table.length)];
             e != null;
             e = e.next) {
            Object k;
            if (e.hash == hash &&
                ((k = e.key) == key || (key != null && 
                	(	key.equals(k) || (key instanceof VertexID && k instanceof CmpVertex && k.equals(key)) )  )))
                return e;
            ++collisionNumber;
        }
        return null;
	...
	public V put(K key, V value) {
        ...
        for (Entry<I,K,V> e = table[i]; e != null; e = e.next) {
            Object k;
    ...
    void resize(int newCapacity) {
        ...
        @SuppressWarnings("unchecked")
		Entry<I,K,V>[] newTable = new Entry[newCapacity];
    ...
    private class EntrySet extends AbstractSet<Map.Entry<K,V>> {
		...
		public boolean contains(Object o) {
            ...
            @SuppressWarnings("unchecked")
			Map.Entry<K,V> e = (Map.Entry<K,V>) o;
            Entry<I,K,V> candidate = getEntry(e.getKey());
    ...
    public V remove(Object key) {
        Entry<I,K,V> e = removeEntryForKey(key);
     ...
     * for this key.
     */
    final Entry<I,K,V> removeEntryForKey(Object key) {
        int hash = (key == null) ? 0 : hash(key);
        int i = indexFor(hash, table.length);
        Entry<I,K,V> prev = table[i];
        Entry<I,K,V> e = prev;

        while (e != null) {
            Entry<I,K,V> next = e.next;
            Object k;
            if (e.hash == hash &&
	...
	public int hashCode() {
        int h = 0;
        for (Map.Entry<K, V> kvEntry : entrySet()) h += kvEntry.hashCode();
        return h;
    }
