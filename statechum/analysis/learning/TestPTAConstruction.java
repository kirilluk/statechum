...
public class TestPTAConstruction {
	...
	public void testAugmentPTA() // only two traces, both accept
	{
		...
		DirectedSparseGraph actualA = new RPNIBlueFringeLearner(null)
				.augmentPTA(RPNIBlueFringeLearner.initialise(), plusStrings,
						true), actualC = new ComputeStateScores(0).setMode(
				IDMode.POSITIVE_NEGATIVE).augmentPTA(plusStrings, true)
				.getGraph();
	...
	private void checkEmptyPTA(String[][] arrayPlusStrings,
		...
		try {
			actualA = new RPNIBlueFringeLearner(null).createAugmentedPTA(
					RPNIBlueFringeLearner.initialise(), plusStrings,
					minusStrings);
		...
		Assert.assertEquals(true, isAccept(((Vertex) actualA.getVertices()
				.iterator().next())));
		Assert.assertEquals(0, actualA.getEdges().size());

		Assert.assertEquals(1, actualC.getVertices().size());
		Assert.assertEquals(true, isAccept(((Vertex) actualC.getVertices()
				.iterator().next())));
	...
	{
		checkEmptyPTA(new String[][] { new String[] {} }, new String[][] {});
	}
	...
	{
		checkEmptyPTA(new String[][] {}, new String[][] {});
	}

	/**
	 * Builds a PTA from the supplied arguments using two different methods. If
	 * any of them throws, checks that another one throws too and then rethrows
	 * the exception.
	 * 
	 * @param arrayPlusStrings
	 *            allowed sequences
	 * @param arrayMinusStrings
	 *            sequences ending at a reject state
	 * @param expectedPTA
	 *            a textual representation of a PTA which should be built.
	 */
	private void checkPTAconstruction(String[][] arrayPlusStrings,
			String[][] arrayMinusStrings, String expectedPTA) {
		Set<List<String>> plusStrings = buildSet(arrayPlusStrings), minusStrings = buildSet(arrayMinusStrings);
		DirectedSparseGraph actualA = null, actualC = null;
		IllegalArgumentException eA = null, eC = null;
		try {
	...
	{
		checkPTAconstruction(new String[][] { new String[] { "a", "b", "c" },
				new String[] { "a", "d", "c" } },
	...
	{
		checkPTAconstruction(new String[][] { new String[] { "a", "b", "c" },
				new String[] { "a", "b" }, new String[] { "a", "d", "c" } },
				new String[][] { new String[] { "a", "b", "c", "d" },
						new String[] { "a", "u" } },
				"A-a->B--b->C-c->End1-d-#REJ\nB--d->C2-c->End2\nB-u-#A2");
	}
