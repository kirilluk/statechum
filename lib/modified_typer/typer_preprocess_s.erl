...
get_all_files(Args, analysis) ->
  case internal_get_all_files(Args#args.analyze,
    ...
    [] -> typer_s:reportError("no file(s) to analyze");
    AllFiles -> AllFiles
  end;
...
check_dir(Dir, Mode, Acc, Fun) ->
  case file:list_dir(Dir) of
    ...
    {error, eacces} ->
      typer_s:reportError("no access permission to dir \""++Dir++"\"");
    {error, enoent} ->
      typer_s:reportError("cannot access "++Dir++": No such file or directory");
    {error, _Reason} ->
      typer_s:reportError("error involving a use of file:list_dir/1")
  end.
