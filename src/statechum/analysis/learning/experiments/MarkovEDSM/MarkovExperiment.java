...
public class MarkovExperiment
{
	...
	public static final String nameComponentSeparator = "_";
	/** We might have pre-generated a number of PTA with varying transition coverage. Select the best one.
	 *
	 * @param nameOfPerfectPTA name of PTA to be considered perfect (100% transition coverage).
	 * @param ptaNames names of PTAs
	 * @return The one with highest transition coverage (as part of file name) as well as the seed that was used to generate it.
	 */
	public static Pair<File,Long> selectBestPTAFromIncomplete(File nameOfPerfectPTA, File [] ptaNames) {
		int bestCoverage = 0;
		long bestSeed = -1;
		File bestFile = null;

		int perfectNameLen = nameOfPerfectPTA.getName().length();
		for(File fileName:ptaNames)
			if (fileName.getParentFile().getAbsolutePath().equals(nameOfPerfectPTA.getParentFile().getAbsolutePath()) && fileName.getName().startsWith(nameOfPerfectPTA.getName()))
			{
				String [] components =  fileName.getName().substring(perfectNameLen).split(nameComponentSeparator);
				if (components.length != 3 || !components[0].isEmpty())
					continue;
				int newCoverage = Integer.parseInt(components[2]);
				if (newCoverage > bestCoverage) {
					bestCoverage = newCoverage;bestSeed = Long.parseLong(components[1]);bestFile = fileName;
				}
			}

		return new Pair<>(bestFile,bestSeed);
	}
	...
	public static class MarkovLearnerRunner extends UASExperiment<MarkovLearningParameters,ExperimentResult<MarkovLearningParameters>>
	{
		...
		public LearnerGraph constructPTA(boolean saveGeneratedPTA, long how_long_it_has_to_take_to_warrant_saving)
		{
			...
			File ptaPreGenerated = new File(filenameForPTA.toFileName());// get the name for 100% transition coverage
			if (!ptaPreGenerated.exists())// if we do not have one, list existing files, looking for the best coverage we got so far
				// (these PTAs are generated by running experiment with COUNT_TASKS_PARALLELPTA or COUNT_TASKS_PTA).

				ptaPreGenerated = selectBestPTAFromIncomplete(
						ptaPreGenerated,
						new File(filenameForPTA.toFileName()).getParentFile().listFiles()).firstElem;

			if (ptaPreGenerated != null && ptaPreGenerated.exists()) {
                ...
                try {
                    AbstractPersistence.loadGraph(ptaPreGenerated, pta, learnerInitConfiguration.getLabelConverter());
                } catch (IOException e) {
				...
				if (pta != null) {
					...
					if (!ptaPreGenerated.equals(new File(filenameForPTA.toFileName())))
						System.out.println("Choosing pre-generated imperfect PTA "+ptaPreGenerated.getName());
					return pta;
				}
            }
				...
				if (currentCoverage > bestCoverage) {
					...
					if (saveGeneratedPTA && how_long_it_has_to_take_to_warrant_saving*6 < LearningSupportRoutines.getThreadTime() - ptaGenerationStartTime) {
						try {
							...
							pta.storage.writeGraphML(filenameForPTA.toFileName()+nameComponentSeparator+attemptCounter+nameComponentSeparator+bestCoverage);
						} catch (IOException e) {
