...
public class TestWMethod {
	...
	/**
	 * Test method for
	 * {@link statechum.xmachine.model.testset.WMethod#getTransitionCover()}.
	 */
	@Test
	public final void computeStateCover1() {
		Set<List<String>> expected = TestFSMAlgo.buildSet(new String[][] {
				new String[] {}, new String[] { "b" },
				new String[] { "b", "a" } }), actual = new HashSet<List<String>>();
		actual.addAll(WMethod.computeStateCover(getGraphData(buildGraph(
	...
	public final void computeStateCover2() {
		Collection<List<String>> expected = TestFSMAlgo
				.buildSet(new String[][] { new String[] {},
						new String[] { "b" }, new String[] { "b", "a" } }), actual = new HashSet<List<String>>();
		actual.addAll(WMethod.computeStateCover(getGraphData(buildGraph(
	...
	public final void computeStateCover3() {
		Collection<List<String>> expected = TestFSMAlgo
				.buildSet(new String[][] { new String[] {},
						new String[] { "b" }, new String[] { "d" },
						new String[] { "b", "a" } }), actual = new HashSet<List<String>>();
		actual.addAll(WMethod
				.computeStateCover(getGraphData(buildGraph(
						"A-a->A\nD<-d-A-b->B-c->B-a->C\nQ-d->S",
						"computeStateCover3"))));
		Assert.assertTrue(expected.equals(actual));
	}
	...
	public final void computeStateCover4() {
		Collection<List<String>> expected = TestFSMAlgo
				.buildSet(new String[][] { new String[] {},
						...
						new String[] { "d" }, new String[] { "b", "a" },
						new String[] { "b", "a", "a" } }), actual = new HashSet<List<String>>();
		actual.addAll(WMethod.computeStateCover(getGraphData(buildGraph(
	...
	/**
	 * Test method for
	 * {@link statechum.xmachine.model.testset.WMethod#isPrefix(java.util.List, java.util.List)}.
	 */
	@Test
	public final void testIsPrefix0() {
		Assert.assertTrue(isPrefix(Arrays
				.asList(new String[] { "a", "b", "c" }), Arrays
				.asList(new String[] {})));
	}
	...
	public final void testIsPrefix2() {
		Assert.assertFalse(isPrefix(Arrays
				.asList(new String[] { "a", "b", "c" }), Arrays
	...
	/**
	 * Test method for
	 * {@link statechum.xmachine.model.testset.WMethod#cross(java.util.Set, java.util.Set)}.
	 */
	@Test
	...
	public final void testCross1b() {
		...
		actual
				.addAll(cross(
						buildList(new String[][] { new String[] {},
								new String[] { "a", "b" },
								new String[] { "z", "x" } }),
						buildList(new String[][] { new String[] {},
								new String[] { "c", "d" },
	...
	public final void testCross2() {
		Set<List<String>> actual = new HashSet<List<String>>();
		actual.addAll(cross(
				buildList(new String[][] { new String[] { "a", "b" } }),
				buildList(new String[][] { new String[] { "c", "d" },
						new String[] { "q", "w" } })));
		Set<List<String>> expected = buildSet(new String[][] {
				new String[] { "a", "b", "c", "d" },
	...
	/**
	 * Test method for
	 * {@link statechum.xmachine.model.testset.WMethod#crossWithSet(java.util.Set, java.util.Set)}.
	 */
	@Test
	public final void testCrossWithSet1() {
		...
		actualA.addAll(crossWithSet(buildList(new String[][] {
				new String[] { "a", "b" }, new String[] { "z", "x" } }), with));
		actualB.addAll(crossWithSet_One(buildList(new String[][] {
				new String[] { "a", "b" }, new String[] { "z", "x" } }), with));
		Set<List<String>> expected = buildSet(new String[][] {
				new String[] { "a", "b", "p" }, new String[] { "z", "x", "p" },
				new String[] { "a", "b", "q" }, new String[] { "z", "x", "q" } });
		Assert.assertTrue("expected: " + expected + " actual: " + actualA,
				expected.equals(actualA));
		Assert.assertTrue("expected: " + expected + " actual: " + actualB,
				expected.equals(actualB));
	}
	...
	public final void testCrossWithSet4() {
		...
		seq.clear();
		seq.addAll(Arrays.asList(new String[] { "a", "b" }));
		data.add((List<String>) seq.clone());
		seq.clear();
		seq.addAll(Arrays.asList(new String[] { "z", "x" }));
		data.add((List<String>) seq.clone());// cannot use buildList here
												// because it returns a
												// collection of immutable
												// sequences

		actualA.addAll(crossWithSet((LinkedList<List<String>>) data.clone(),
				with));
		actualB.addAll(crossWithSet_One(
				(LinkedList<List<String>>) data.clone(), with));
		Set<List<String>> expected = buildSet(new String[][] {
				new String[] { "a", "b", "p" }, new String[] { "z", "x", "p" } });
		Assert.assertTrue("expected: " + expected + " actual: " + actualA,
				expected.equals(actualA));
	...
	/**
	 * Test method for
	 * {@link statechum.xmachine.model.testset.WMethod#getStimuli}.
	 */
	@Test
	public final void testGetStimuli1() {
		Assert.assertTrue(WMethod.makeSingleton(
				WMethod.computeAlphabet(new DirectedSparseGraph())).isEmpty());
	}
	...
	public final void testGetStimuli2() {
		...
		singleton.addAll(WMethod.makeSingleton(WMethod
				.computeAlphabet(buildGraph("A-p->A-b->B-c->B-a->C\nQ-d->S",
						"testGetStimuli2"))));
		Assert.assertTrue(singleton.equals(buildSet(new String[][] {
				new String[] { "p" }, new String[] { "d" },
	...
	public final void testComputeFSMAlphabet1() {
		...
		expected.add("p");
		Assert.assertTrue(WMethod.computeAlphabet(
				getGraphData(buildGraph("A-p->A", "testComputeFSMAlphabet1")))
				.equals(expected));
	...
	public final void testComputeFSMAlphabet2() {
		...
		expected.add("a");
		Assert.assertTrue(WMethod.computeAlphabet(
				getGraphData(buildGraph("A-p->A-b->B-c->B-a->C\nQ-d->S",
						"testComputeFSMAlphabet2"))).equals(expected));

	}

	@Test
	public final void testTruncateSeq1() {
		FSMStructure fsm = getGraphData(buildGraph(
				"A-p->A-b->B-c->B-a->C\nQ-d->S", "testTruncateSeq1"));
		Assert.assertEquals(Arrays.asList(new String[] { "p", "b" }),
				WMethod.truncateSequence(fsm, Arrays.asList(new String[] { "p",
						"b" })));
	}
	...
	public final void testTruncateSeq2() {
		...
		Assert.assertEquals(Arrays.asList(new String[] { "p", "b", "c" }),
				WMethod.truncateSequence(fsm, Arrays.asList(new String[] { "p",
						"b", "c" })));
	}
	...
	public final void testTruncateSeq3() {
		...
		Assert.assertEquals(Arrays.asList(new String[] {}), WMethod
				.truncateSequence(fsm, Arrays.asList(new String[] {})));
	}
	...
	public final void testTruncateSeq4() {
		...
		Assert.assertEquals(Arrays.asList(new String[] { "p", "b", "z" }),
				WMethod.truncateSequence(fsm, Arrays.asList(new String[] { "p",
						"b", "z", "e" })));
	}
	...
	public final void testTruncateSeq5() {
		...
		Assert.assertEquals(Arrays.asList(new String[] { "p", "b", "d" }),
				WMethod.truncateSequence(fsm, Arrays.asList(new String[] { "p",
						"b", "d" })));
	}
	...
	public final void testTruncateSeq6() {
		...
		Assert.assertEquals(Arrays.asList(new String[] { "a" }), WMethod
				.truncateSequence(fsm, Arrays.asList(new String[] { "a" })));
	}
	...
	public final void testTruncateSeq7() {
		...
		Assert.assertEquals(Arrays.asList(new String[] { "p", "p", "p" }),
				WMethod.truncateSequence(fsm, Arrays.asList(new String[] { "p",
						"p", "p" })));
	}
	...
	/**
	 * Test method for
	 * {@link statechum.xmachine.model.testset.WMethod#appendSequence(statechum.analysis.learning.TestFSMAlgo.FSMStructure, java.util.Set, java.util.List)}.
	 */
	@Test
	public final void testAppendSequence0() {
		...
		Set<List<String>> actual = new HashSet<List<String>>();
		actual.addAll(sequences.getData());
	...
	public final void testAppendSequence5() {
		...
		sequences.addSequence(Arrays.asList(new String[] { "p", "b" }));
		Set<List<String>> expected = buildSet(new String[][] {
				new String[] { "p", "b" }, new String[] { "p", "p", "p" } });

		WMethod.appendSequence(fsm, sequences, Arrays
				.asList(new String[] { "p" }));
		WMethod.appendSequence(fsm, sequences, Arrays.asList(new String[] {
				"p", "p" }));
		WMethod.appendSequence(fsm, sequences, Arrays.asList(new String[] {
	...
	/**
	 * Test method for
	 * {@link statechum.xmachine.model.testset.WMethod#appendAllSequences(statechum.analysis.learning.TestFSMAlgo.FSMStructure, java.util.Set, java.util.Set)}.
	 */
	@Test
	...
	public final void testAppendAllSequences0b() {
		...
		WMethod
				.appendAllSequences(fsm, sequences,
						buildList(new String[][] {}));
		Set<List<String>> actual = new HashSet<List<String>>();
		actual.addAll(sequences.getData());
		Assert.assertTrue("expected : " + expected + " got: " + actual,
	...
	public static void testWsetconstruction(String machine,
			boolean equivalentExpected, boolean reductionExpected) {
		...
		try {
			Set<List<String>> wset = new HashSet<List<String>>();
			wset.addAll(WMethod.computeWSet(fsm));
			Assert.assertEquals(false, equivalentExpected);
	...
	/**
	 * Given an FSM and a W set, checks if it is a valid W set and throws if
	 * not.
	 * 
	 * @param fsm
	 *            the machine
	 * @param wset
	 *            the set to check validity of.
	 */
	private static void checkW_is_corrent(FSMStructure fsm,
			Collection<List<String>> wset) {
			...
			for (Entry<String, Boolean> stateB : fsm.accept.entrySet())
				if (!stateA.getKey().equals(stateB.getKey())) {
					...
					while (pathIt.hasNext() && !foundString) {
						...
						int aResult = WMethod.tracePath(fsm, path, stateA
								.getKey()), bResult = WMethod.tracePath(fsm,
								path, stateB.getKey());
					...
					if (!foundString)
						fail("W set does not distinguish between "
								+ stateA.getKey() + " and " + stateB.getKey());
				}
		}
	}
	...
	public final void testWset10() {
		...
		int a = 4, b = 2;
		machine = machineOrig.replaceAll(a + "--", "Q" + "--").replaceAll(
				">" + a, ">" + "Q").replaceAll(b + "--", a + "--").replaceAll(
				">" + b, ">" + a).replaceAll("Q" + "--", b + "--").replaceAll(
				">" + "Q", ">" + b);
	...
	/**
	 * Adds an entry to the supplied map of pairs of states to labels which
	 * distinguish among those states.
	 * 
	 * @param distinguishingLabels
	 *            map of pairs of states to labels which distinguish among those
	 *            states
	 * @param stateA
	 *            first state
	 * @param stateB
	 *            second state
	 * @param labels
	 *            labels to distinguish between states.
	 */
	private final static void addDistLabels(
			Map<String, Map<String, Set<String>>> distinguishingLabels,
			String stateA, String stateB, String[] labels) {
		Set<String> distLabels = new HashSet<String>();
		Map<String, Set<String>> stateToDist = new HashMap<String, Set<String>>();
		distLabels.clear();
		distLabels.addAll(Arrays.asList(labels));
		stateToDist.put(stateB, distLabels);
	...
	public final void testComputeTopLabe5() {
		...
		addDistLabels(distinguishingLabels, "0", "1", new String[] { "b" });
		addDistLabels(distinguishingLabels, "3", "1", new String[] { "b" });
		addDistLabels(distinguishingLabels, "4", "1", new String[] {});// empty
																		// entry
		Assert.assertEquals("b", WMethod.computeTopLabel(distinguishingLabels));
	}
	...
	public final void testCheckEquivalentStates3() // no equivalent states
	{
		...
		FSMStructure fsm = TestFSMAlgo.convertTableToFSMStructure(table,
				new int[] { 0, 1 }, -1);
		Assert.assertEquals(false, WMethod.checkEquivalentStates(fsm));
	}
	...
	private void checkTestGenerationResult(WMethod testGenerator,
			String[][] expected) {
		Set<List<String>> expectedSet = buildSet(expected), actualA = new HashSet<List<String>>(), actualB = new HashSet<List<String>>();
		actualA.addAll(testGenerator.computeOldTestSet());
		actualB.addAll(testGenerator.computeNewTestSet());
		assertTrue("old test generator failure, received " + actualA
				+ " expected " + expectedSet, expectedSet.equals(actualA));
		assertTrue("new test generator failure, received " + actualB
				+ " expected " + expectedSet, expectedSet.equals(actualB));
	}
	...
	public final void testTestGeneration5() {
		WMethod wm = new WMethod(buildGraph("A-a->A-b->B",
				"testTestGeneration5"), 1);
		checkTestGenerationResult(wm, new String[][] {
				new String[] { "a", "a", "a" }, new String[] { "a", "b", "a" },
				new String[] { "b", "a" }, new String[] { "b", "b" } });
	}

	@Test
	public final void testTestGeneration6() {
		WMethod wm = new WMethod(buildGraph("S-p->A-a->A1-a->A3\n"
				+ "A-b->A2-b->A3\nA<-c-A2<-c-A3\n"
				+ "A<-d-A5<-a-A3-b->A4-f->AA4-a->S\n" + "A-d->A\nA5-b->AA6",
				"testTestGeneration6"), 0);
		Set<List<String>> actualA = new HashSet<List<String>>(), actualB = new HashSet<List<String>>();
		actualA.addAll(wm.computeOldTestSet());
		actualB.addAll(wm.computeNewTestSet());
		assertTrue(
				"the two test generators return different values, old returns "
						+ actualA + " and the new one - " + actualB, actualA
						.equals(actualB));
	}
	...
	public final void testCheckUnreachable1() {
		assertFalse(WMethod.checkUnreachableStates(WMethod
				.getGraphData(buildGraph("A-a->A", "testCheckUnreachable1"))));
	}

	@Test
	public final void testCheckUnreachable2() {
		assertFalse(WMethod.checkUnreachableStates(WMethod
				.getGraphData(buildGraph("A-a->A-c->C\nB-a->A\nC-b->B",
						"testCheckUnreachable2"))));
	}

	@Test
	public final void testCheckUnreachable3() {
		assertTrue(WMethod.checkUnreachableStates(WMethod
				.getGraphData(buildGraph("A-a->A-c->C\nB-a->A",
						"testCheckUnreachable3"))));
	}
