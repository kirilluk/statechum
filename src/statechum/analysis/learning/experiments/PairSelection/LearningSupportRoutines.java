...
public class LearningSupportRoutines
{
	...
	public static Map<Label,Pair<CmpVertex,CmpVertex> > uniqueIntoState(LearnerGraph graph)
	{
		...
		Map<Label, Pair<CmpVertex,CmpVertex> > labelToStatePair = new TreeMap<>();
			...
			for(Entry<Label,CmpVertex> target:entry.getValue().entrySet())
				...
				{// the label is not already recorded as leading to multiple different states.
					...
					if (recordedSourceTarget == null)
						...
						labelToStatePair.put(target.getKey(), new Pair<>(entry.getKey(), target.getValue()));
					else
						if (recordedSourceTarget.secondElem != target.getValue())
						{
							...
							labelToStatePair.put(target.getKey(), new Pair<>(null, recordedSourceTarget.secondElem));
				}
	...
	public static Label uniqueFromInitial(LearnerGraph graph)
	{
		if (graph.getInit() == null)
			return null;
		Set<Label> liveLabels = new HashSet<>();liveLabels.addAll(graph.transitionMatrix.get(graph.getInit()).keySet());
	...
	public static void addIfThenForMandatoryMerge(LearnerEvaluationConfiguration initialData, Collection<Label> dataOnUniqueTransitions)
	{
		if (initialData.ifthenSequences == null)
			initialData.ifthenSequences = new LinkedList<>();
	...
	public static void addIfThenForPairwiseConstraints(LearnerEvaluationConfiguration initialData, Map<Label,Set<Label>> pairwiseConstraints)
	{
		...
		for(Entry<Label,Set<Label>> entry:pairwiseConstraints.entrySet())
			if (!entry.getValue().isEmpty())
			{
				...
				initialData.ifthenSequences.add(QSMTool.cmdIFTHENAUTOMATON + " "+ pairwiseAutomata+"_"+lbl+" A- !"+lbl+" ->A-"+lbl+"->B - "+lbl+" ->B / B- !"+lbl+" ->A "+ rejects +"/ B == THEN == C");
			}
	}
	...
	{
		List<CmpVertex> sourceStates = new LinkedList<>(), statesOfInterest = new LinkedList<>();
	...
	public static LearnerGraph mergeStatesForUnique(LearnerGraph pta, Label unique)
	{
		...
		List<StatePair> pairs = new LinkedList<>();
	...
	public static boolean checkForMerge(PairScore pair, LearnerGraph tentativeGraph)
	{
		...
		for(Entry<Label,CmpVertex> entry:tentativeGraph.transitionMatrix.get(pair.getQ()).entrySet()) // iterate over the smaller set
			...
			{
				if (labelsOfInterest == null) labelsOfInterest = new TreeSet<>();
	...
	{
		Map<Label,Set<Label>> labelToSet = new TreeMap<>();
		Set<Label> alphabet = tentativeGraph.pathroutines.computeAlphabet();
		for(Label lbl:alphabet)
		{
			Set<Label> labels = new TreeSet<>();labels.addAll(alphabet);labelToSet.put(lbl,labels);
		}
	...
	public static List<PairScore> filterPairsBasedOnMandatoryMerge(List<PairScore> pairs, LearnerGraph tentativeGraph,Collection<Label> labelsLeadingToStatesToBeMerged,Collection<Label> labelsLeadingFromStatesToBeMerged)
	{
		...
		List<PairScore> outcome = new ArrayList<>();
	...
	{
		List<StatePair> pairsList = new ArrayList<>();
		if (transitionsToTheSameState.isEmpty() && transitionsFromTheSameState.isEmpty() )
			return pairsList;
		
		Map<Label,Collection<CmpVertex>> labelToStates = 
				tentativeGraph.config.getTransitionMatrixImplType() == STATETREE.STATETREE_ARRAY? new ArrayMapWithSearchPos<>() : new TreeMap<>();
		Map<Label,Collection<CmpVertex>> labelFromStates = 
				tentativeGraph.config.getTransitionMatrixImplType() == STATETREE.STATETREE_ARRAY? new ArrayMapWithSearchPos<>() : new TreeMap<>();
					
		for(Label lbl:transitionsToTheSameState) labelToStates.put(lbl, new ArrayList<>());
		for(Label lbl:transitionsFromTheSameState) labelFromStates.put(lbl, new ArrayList<>());
		for(Entry<CmpVertex,MapWithSearch<Label,Label,CmpVertex>> entry:tentativeGraph.transitionMatrix.entrySet())
			if (entry.getKey().isAccept())
				for(Entry<Label,CmpVertex> transition:entry.getValue().entrySet())
				{
	...
	public static List<StatePair> buildVerticesToMergeForPathsFrom(LearnerGraph tentativeGraph, Label transitionFromTheSameState)
	{
		...
		List<CmpVertex> statesToMerge = new ArrayList<>();
		for(Entry<CmpVertex, MapWithSearch<Label, Label, CmpVertex>> entry:tentativeGraph.transitionMatrix.entrySet())
			if (entry.getKey().isAccept())
			{
	...
	static void updateStatistics( Map<Long,TrueFalseCounter> pairQuality, LearnerGraph tentativeGraph, LearnerGraph referenceGraph, Collection<PairScore> pairsToConsider)
	{
		...
		{
			List<PairScore> correctPairs = new ArrayList<>(pairsToConsider.size()), wrongPairs = new ArrayList<>(pairsToConsider.size());
	...
	{
		Map<CmpVertex,Set<Label>> outcome = new TreeMap<>();
		StatePair reference_pta = new StatePair(reference.getInit(),pta.getInit());
		LinkedList<StatePair> pairsToExplore = new LinkedList<>();pairsToExplore.add(reference_pta);
		for(Entry<CmpVertex, MapWithSearch<Label, Label, CmpVertex>> entry:reference.transitionMatrix.entrySet())
			outcome.put(entry.getKey(), new TreeSet<>(entry.getValue().keySet()));
		Set<CmpVertex> visitedInTree = new HashSet<>();
	...
	public static LearnerGraph checkIfSingleStateLoopsCanBeFormed(MarkovClassifierLG ptaClassifier,LearnerGraph referenceGraph,final Collection<List<Label>> pathsOfInterest)
	{
		...
		int nrOfMergers=0;
		List<StatePair> pairsToMerge = new LinkedList<>();
		for(Entry<CmpVertex, MapWithSearch<Label, Label, CmpVertex>> entry:merged.transitionMatrix.entrySet())
			for(Entry<Label,CmpVertex> transition:entry.getValue().entrySet())
				...
				{// we have a potential loop
					...
					verticesToMerge = new LinkedList<>();
					...
					{
						System.out.println( p +(wrongPairs.isEmpty()?"valid, ":"invalid:")+value+ "(score "+genScore+")");
	...
	{
		Set<CmpVertex> statesUniquelyIdentified = new TreeSet<>();
	...
	public static Collection<CmpVertex> labelStatesAwayFromRoot(LearnerGraph graph, int steps)
	{
		...
		Collection<CmpVertex> frontLine = new LinkedList<>(), nextLine = new LinkedList<>(), previousFrontLine = null;
		...
		for(int line=0;line < steps;++line)
		{
			...
			previousFrontLine = frontLine;frontLine = nextLine;nextLine= new LinkedList<>();
		}
	...
	{
		List<StatePair> listOfPairs = new LinkedList<>();
