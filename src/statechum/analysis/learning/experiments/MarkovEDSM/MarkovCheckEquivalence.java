...
public class MarkovCheckEquivalence {
    ...
    public static void main(String []args) {
        ...
        String description = "equivalence_check";
        ...
        if (curPhase == SGE_ExperimentRunner.PhaseEnum.COLLECT_AVAILABLE || curPhase == SGE_ExperimentRunner.PhaseEnum.COLLECT_RESULTS) {// by the time we are here, experiments for the current number of states have completed, hence record the outcomes.
            Set<RESULT_VALUES> invalidCellValuesA = obtainValidityOfCellValues(description,twoExperiments.get(0));checkFullTransitionCoverageAttained(experimentsToCompare[0], twoExperiments.get(0), invalidCellValuesA);
            Set<RESULT_VALUES> invalidCellValuesB = obtainValidityOfCellValues(description,twoExperiments.get(1));checkFullTransitionCoverageAttained(experimentsToCompare[1], twoExperiments.get(1), invalidCellValuesB);
