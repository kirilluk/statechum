...
public class ExtendedSequenceFindLoopsHandler extends SequenceHandler {
	...
	public ExtendedSequenceFindLoopsHandler(Map<String,List<TreePath>> functions, ClassMethodDefsHandler classMethods){
	...
	private Integer getMappingInteger(Integer classId){
		int index = 0;
		List<Integer> processesForFunction = observationsToProcesses.get(sequenceClone);
		...
		else index = processesForFunction.indexOf(classId);
		if(index<0){
			processesForFunction.add(classId);
			observationsToProcesses.put(sequenceClone, processesForFunction);
	...
	private void addToFindloopsObservation(){
		List<List<String>> stringList = abstractFunctionsToObservations.get(triggerKey);
		if(stringList == null)
		...
		for(int i=1;i<sequenceClone.size();i++){
			...
			observation.add("s"+"\t"+processForSenderClass+"\t"+processForCurrentClass+"\t"+ticketToString.get(currentTicket)+"\n");
			observation.add("r"+"\t"+processForSenderClass+"\t"+processForCurrentClass+"\t"+ticketToString.get(currentTicket)+"\n");
		}
		stringList.add(observation);
		this.stringListToIntegerList.put(observation, observationsToProcesses.get(sequenceClone));
		abstractFunctionsToObservations.put(triggerKey, stringList);
		triggerKey = null; this.sequenceClone.clear();
	}
	
	@Override
	public void endDocument() throws SAXException {
		super.endDocument();
		Iterator<String> keyIt = this.abstractFunctionsToObservations.keySet().iterator();
		while(keyIt.hasNext()){
			String key = keyIt.next();
			List<List<String>> observations = abstractFunctionsToObservations.get(key);
			int counter = 0;
			for (List<String> list : observations) {
				counter++;
				try{
					FileOutputStream fos = new FileOutputStream("findloopsInput"+System.getProperty("path.separator")+key+counter);
