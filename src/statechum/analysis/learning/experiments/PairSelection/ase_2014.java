...
public class ase_2014 extends PairQualityLearner
{
		...
		public static void throwExceptionIfTooManyReds( LearnerGraph graph )
		{
			...
			for(CmpVertex v:graph.transitionMatrix.keySet())
				if (v.getColour() == JUConstants.RED)
					if (countOfRed++ > 200) // threshold to abort learning, set at 4x the maximal number of states (50) we'd like to learn from.
	...
	public static class LearnerRunner implements Callable<ThreadResult>
	{
		...
		protected int traceMultiplier = 1;
		...
		public void setTraceMultiplier(int traceMulti) {
			traceMultiplier=traceMulti;
		}
		...
		public ThreadResult call() throws Exception 
		{
			...
			final Collection<List<Label>> testSet = PaperUAS.computeEvaluationSet(referenceGraph,states*3,makeEven(states*alphabet*traceMultiplier));
	...
	public static class EDSM_MarkovLearner extends LearnerThatCanClassifyPairs implements statechum.analysis.learning.rpnicore.PairScoreComputation.RedNodeSelectionProcedure
	{
		...
		public long overrideScoreComputation(PairScore p) 
		{
			...
			{			
				...
				if(coregraph.getStateNumber()-merged.getStateNumber() == 1 && p.getR().getIntegerID() < Markov.getChunkLen())
				{
