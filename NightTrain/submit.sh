...
if [ `expr $DATASETNO % 100` != 0 ];then echo Job number has to be divisible by 100.; exit;fi
COUNT=`expr $DATASETNO / 100`
mkdir -p $OUTPUT 
rm $OUTPUT/*
/bin/date > $OUTPUT/started
# The first argument to tasks is the machines file and the second one is the output directory
qsub -cwd -l qp=LOW -l mclass=c1 -t 1-`expr 1 '+' $DATASETNO`:$COUNT -N $JOBNAME -j y -o $OUTPUT/LOG learner.sh $MACHINES $OUTPUT $COUNT || exit 
