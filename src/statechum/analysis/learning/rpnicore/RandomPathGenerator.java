...
public class RandomPathGenerator {
	 ...
	 */ 
	protected Map<CmpVertex,List<Label>> constructShortestPathsToInit()
	{
	...
	private Entry<Label,CmpVertex> pickNextState(CmpVertex currentState,int lengthMax)
	{
			...
			{// if we are asked to generate negative paths of length 1, we cannot start with anything positive.
				for(int i=0;i<positiveLength;i++)
				{
					...
					if (positive)
					{
						...
						// In case of overshoot, the step may get us further away from the target, hence we should not make it even if it is within delta of the target.
						...
						inputState = pickNextState(current,positiveLength-i+ Math.min(deltaInGenerationOfWalkLeadingToTheInitialState, positiveLength-currentLength-1));
						if (inputState == null)
