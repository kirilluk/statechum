...
public class AccuracyAndQuestionsExperiment {
	...
	public abstract static class LearnerEvaluator implements Callable<String> {
		...
		protected void buildSetsHalfNegative() {
			...
			RandomPathGenerator rpg = new RandomPathGenerator(graph,
					new Random(100), size / 2, 5);// the seed for Random
			...
			currentSamples = addNumberFromSamples(currentSamples, rpg
					.getAllPaths(), number / 2);
			currentSamples = addNumberFromSamples(currentSamples, rpg
					.getNegativePaths(), number / 2);
			sPlus = getPositiveStrings(graph, currentSamples);

			sMinus = getNegativeStrings(graph, currentSamples);
			/*
			 * FSMStructure fsm = WMethod.getGraphData(graph); for(int i=0;i<size;i++){
			 * sMinus.add(pickNegativeTest(fsm)); }
			 */
			System.out.println("total at this percentage: "
					+ currentSamples.size() + ", plus : " + sPlus.hashCode()
					+ "-" + sPlus.size() + " minus: " + sMinus.hashCode() + "-"
					+ sMinus.size());
		}
		...
		public List<String> pickNegativeTest(FSMStructure graph) {
			...
			while (accepted) {
				...
				string = WMethod.trimSequence(graph,
						(List<String>) testArray[randomIndex]);
				if (string == null)
					continue;
		...
		protected void buildSets() {
			...
			RandomPathGenerator rpg = new RandomPathGenerator(graph,
					new Random(100), size, 5);// the seed for Random should be
												// the same for each file
			WMethod tester = new WMethod(graph, 2);
			tests = (Collection<List<String>>) tester.getFullTestSet();
			// tests = randomHalf(fullTestSet,new Random(0));
			// Collection<List<String>> fullSampleSet =
			// WMethod.crossWithSet(rpg.getAllPaths(),WMethod.computeAlphabet(graph));

			// this one ensures that walks are of length diameter+5 if they
			// exist and some will not exist
			// l.setCertaintyThreshold(2);
			// Collection<List<String>> sampleSet = randomHalf(fullSampleSet,new
			// Random(1));
			...
			// System.out.println("total at this percentage:
			// "+currentSamples.size()+", plus :
			// "+sPlus.hashCode()+"-"+sPlus.size()+" minus:
			// "+sMinus.hashCode()+"-"+sMinus.size());
		}
	...
	public static abstract class RPNIEvaluator extends LearnerEvaluator {
		...
		public String call() {
			...
			String result = "";
			String stats = "Instance: " + instanceID + ", learner: " + this
					+ ", sPlus: " + sPlus.size() + " sMinus: " + sMinus.size()
					+ " tests: " + tests.size() + "\n";
			try {
				PTASequenceSet plusPTA = new PTASequenceSet();
				plusPTA.addAll(sPlus);
				PTASequenceSet minusPTA = new PTASequenceSet();
				minusPTA.addAll(sMinus);
				stats = stats + "Actual sequences, sPlus: " + plusPTA.size()
						+ " sMinus: " + minusPTA.size() + " ";
				...
				result = result + l.getQuestionCounter() + FS
						+ computeAccuracy(learningOutcome, graph, tests);
				if (this.percent == 10)
					System.out.println();
				...
				if (learningOutcome != null)
					stats = stats
							+ (learningOutcome
									.containsUserDatumKey(JUConstants.STATS) ? "\n"
									+ learningOutcome.getUserDatum(
											JUConstants.STATS).toString()
									: "");
				// System.out.println(inputFileName+" (instance "+instanceID+"),
				// learner "+this+", "+ percent+"% terminated at
				// "+Calendar.getInstance().getTime());
	...
	private static double computeAccuracy(DirectedSparseGraph learned,
	...
	public static Set<List<String>> addNumberFromSamples(
			Set<List<String>> current, Collection<List<String>> samples,
			double number) {
		double size = samples.size();
		List<String>[] sampleArray = (List<String>[]) samples
	...
	public static Collection<List<String>> getNegativeStrings(
			DirectedSparseGraph graph, Collection<List<String>> samples) {
		Iterator<List<String>> sampleIt = samples.iterator();
	...
	public static final LearnerEvaluatorGenerator[] learnerGenerators = { new LearnerEvaluatorGenerator() {
		 ...
		 * , new LearnerEvaluatorGenerator() { @Override LearnerEvaluator
		 * getLearnerEvaluator(String inputFile, String outputDir, int percent,
		 * int instanceID) { return new RPNIEvaluator(inputFile,outputDir,
		 * percent, instanceID) { @Override protected void
		 * changeParametersOnLearner(RPNIBlueFringeLearner l) { }
		 * 
		 * @Override protected void
		 * changeParametersOnComputeStateScores(ComputeStateScores c) {
		 * c.setMode(IDMode.POSITIVE_ONLY); }
		 * 
		 * @Override public String toString() { return "RPNI, POSITIVE_ONLY"; } }; } },
		 * new LearnerEvaluatorGenerator() { @Override LearnerEvaluator
		 * getLearnerEvaluator(String inputFile, String outputDir, int percent,
		 * int instanceID) { return new RPNIEvaluator(inputFile,outputDir,
		 * percent, instanceID) { protected void
		 * changeParametersOnLearner(RPNIBlueFringeLearner l) { }
		 * 
		 * @Override protected void
		 * changeParametersOnComputeStateScores(ComputeStateScores c) {
		 * c.bumpPositive(); c.setMode(IDMode.POSITIVE_ONLY); }
		 * 
		 * @Override public String toString() { return "RPNI, POSITIVE_ONLY with
		 * a bump of 1"; } }; } }, new LearnerEvaluatorGenerator() { @Override
		 * LearnerEvaluator getLearnerEvaluator(String inputFile, String
		 * outputDir, int percent, int instanceID) { return new
		 * RPNIEvaluator(inputFile,outputDir, percent, instanceID) { protected
		 * void changeParametersOnLearner(RPNIBlueFringeLearner l) { }
		 * 
		 * @Override protected void
		 * changeParametersOnComputeStateScores(ComputeStateScores c) {
		 ...
		 * 
		 * @Override public String toString() { return "RPNI, POSITIVE_ONLY with
		 * a bump of 1 and compatibility scores"; } }; } }
		 */
	...
	public static void main(String[] args) {
		...
		if (args.length < 2) {
			...
			// System.getProperNty("file.separator")+"TestGraphs"+System.getProperty("file.separator")
			// +"50-6");
			...
			if (outputDir.canRead() || outputDir.mkdirs()) {
				experiment = new AccuracyAndQuestionsExperiment(outputDir
						.getAbsolutePath());
