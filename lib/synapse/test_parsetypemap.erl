...
parseTypeMap_test_() ->
	...
	{inparallel,
	[
			fun() -> testsynapse:useworker(fun(Pid,Ref) -> Pid!{Ref,addTypeInformation,a},receive {Ref,failure,Text} -> ?assertEqual(true,testsynapse:contains(Text,"Atom cannot be cast to ")),?assertEqual(true,testsynapse:contains(Text,"com.ericsson.otp.erlang.OtpErlangList")) end end) end,
...
%%				Pid!{Ref,addTypeInformation,[{FunToReplace,{Path,Line,FunName,FunArity,{'Func',[],[],{'Any',[]}} }}]},receive {Ref,failure,Text} -> io:format(user,"Failure received: ~p~n",[Text]) end
		...
		fun() -> testsynapse:useworker(fun(Pid,Ref) -> 
				...
				Pid!{Ref,addTypeInformation,[{FunToReplace,{Path,Line,"differentName",FunArity,{'Func',[],[],{'Any',[]}} }}]},receive {Ref,ok,TextType} -> ok end,
				Pid!{Ref,learnErlang,'ErlangExamples/frequency/frequencyBroken.erl'},
				receive {Ref,failure,Text} ->?assertEqual(true,testsynapse:contains(Text,"com.ericsson.otp.erlang.OtpErlangString cannot be cast to ")),?assertEqual(true,testsynapse:contains(Text,"com.ericsson.otp.erlang.OtpErlangAtom")) end 
				%%io:format(user,"Failure received: ~p~n",[Text]) end
		 end end) end,
