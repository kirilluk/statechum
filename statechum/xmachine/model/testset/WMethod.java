...
public class WMethod {
	...
	/**
	 * Builds fsm structures corresponding to the Jung graph passed as an
	 * argument. The aim is to use the constructed structure for the comparison
	 * of different Jung graphs.
	 * 
	 * @param g
	 *            graph from which to extract data
	 * @return the class storing transition information.
	 */
	public static FSMStructure getGraphData(DirectedSparseGraph g) {
		Iterator<DirectedSparseEdge> edgeIt = (Iterator<DirectedSparseEdge>) g
				.getEdges().iterator();
		...
		while (edgeIt.hasNext()) {
			...
			String from = (String) fromVertex.getUserDatum(JUConstants.LABEL), to = (String) toVertex
					.getUserDatum(JUConstants.LABEL);
			Map<String, String> labelToTargetState = extractedFSM.trans
					.get(from);
			if (labelToTargetState == null) {
				labelToTargetState = new TreeMap<String, String>();
				extractedFSM.trans.put(from, labelToTargetState);
			}

			createLabelToStateMap((Set<String>) edge
					.getUserDatum(JUConstants.LABEL), to, labelToTargetState);
		}
	...
	/**
	 * Given a set of labels and a target state, this method adds to a supplied
	 * map an association of every label with the specified target state.
	 * 
	 * @param labels
	 *            labels
	 * @param to
	 *            target state
	 * @param map
	 *            a map associating state <i>to</i> with each of the labels. If
	 *            this is <i>null</i>, a new map is created.
	 * @return an updated map.
	 */
	public static Map<String, String> createLabelToStateMap(
			Collection<String> labels, String to, Map<String, String> map) {
		Map<String, String> result = (map == null) ? new LinkedHashMap<String, String>()
				: map;
		for (String label : labels) {
			if (result.containsKey(label))
				throw new IllegalArgumentException(
						"nondeterminism detected for label " + label);
			result.put(label, to);
		}
		return result;
	}
	...
	public static int tracePath(FSMStructure fsm, List<String> path,
		...
		for (String label : path) {
			...
			if (exitingTrans == null
					|| (current = exitingTrans.get(label)) == null)
				return pos;
		}
		return fsm.accept.get(current).booleanValue() ? RPNIBlueFringeLearner.USER_ACCEPTED
				: pos;
	}
	 ...
	 */
	public static List<String> truncateSequence(FSMStructure fsm,
			List<String> path) {
		int pos = tracePath(fsm, path);
		List<String> seq = path;
		assert (pos == RPNIBlueFringeLearner.USER_ACCEPTED || pos < path.size());
		if (pos >= 0)
			seq = path.subList(0, pos + 1);// up to a rejected position plus
											// one
		return seq;
	}

	/**
	 * Given a database of sequences and a test sequence, converts a given
	 * sequence into a fundamental test sequence and appends a the result to the
	 * <em>sequences</em> set if the fundamental test sequence is not a prefix
	 * of an existing sequence in that set.
	 */
	public static void appendSequence(FSMStructure fsm,
			PrefixFreeCollection sequences, List<String> path) {
		List<String> seq = truncateSequence(fsm, path);
		sequences.addSequence(seq);
	}

	public static void appendAllSequences(FSMStructure fsm,
			PrefixFreeCollection sequences, List<List<String>> paths) {
	...
	Collection<List<String>> computeOldTestSet() {
		...
		if (characterisationSet.isEmpty())
			characterisationSet.add(Arrays.asList(new String[] {}));
		transitionCover = crossWithSet(partialSet, alphabet);
		transitionCover.addAll(partialSet);

		SlowPrefixFreeCollection testsequenceCollection = new SlowPrefixFreeCollection();

		appendAllSequences(fsm, testsequenceCollection, cross(partialSet,
				characterisationSet));
	...
	Collection<List<String>> computeNewTestSet() {
		...
		transitionCover.addAll(stateCover);
	...
	public static List<List<String>> cross(Collection<List<String>> a,
	...
	public static List<List<String>> crossWithSet_One(List<List<String>> a,
			Collection<String> b) {
		if (b.size() == 1) {
	...
	public static List<List<String>> crossWithSet(Collection<List<String>> a,
			Collection<String> b) {
		List<List<String>> returnVect = new LinkedList<List<String>>();
		for (List<String> elemA : a) {
			for (String elemB : b) {
	...
	public static List<List<String>> computeStateCover(FSMStructure fsm) {
		...
		Set<String> statesInFringe = new HashSet<String>();// in order not to
															// iterate through
															// the list all the
															// time.
		Map<String, LinkedList<String>> stateToPath = new HashMap<String, LinkedList<String>>();
		stateToPath.put(fsm.init, new LinkedList<String>());
		fringe.add(fsm.init);
		statesInFringe.add(fsm.init);
			...
			if (targets != null && !targets.isEmpty())
				for (Entry<String, String> labelstate : targets.entrySet()) {
					...
					if (!statesInFringe.contains(target)) {
						LinkedList<String> newPath = (LinkedList<String>) currentPath
								.clone();
		...
		outcome.addAll(stateToPath.values());
		return outcome;
	}
	...
	public static class EquivalentStatesException extends Error {
		...
		public EquivalentStatesException(String a, String b, int Number) {
			stateA = a;
			stateB = b;
			NumberOfEquivalentStates = Number;
		}

		@Override
		public String toString() {
			return "There are " + NumberOfEquivalentStates
					+ " equivalent states, such as " + stateA + " and "
					+ stateB;
		}
	}

	/**
	 * Computes a set of inputs which distinguish a given pair of states, when
	 * supplied with a map between states and their equivalence classes.
	 * 
	 */
	private static Set<String> computeDistinguishingLabel(String stateA,
			String stateB, Map<String, Map<String, Integer>> newMap) {
		Map<String, Integer> mapA = newMap.get(stateA), mapB = newMap
				.get(stateB);
	...
	/**
	 * Computes a characterising set, assuming that there are no unreachable
	 * states (with unreachable states, it will take a bit longer to perform the
	 * computation.
	 * 
	 * @param fsm
	 ...
	 */
	public static List<List<String>> computeWSetOrig(FSMStructure fsm)
			throws EquivalentStatesException {
		...
		for (Entry<String, Integer> stateA : equivalenceClasses.entrySet()) {
			...
			Iterator<Entry<String, Integer>> stateB_It = equivalenceClasses
					.entrySet().iterator();
			while (stateB_It.hasNext()) {
				Entry<String, Integer> stateB = stateB_It.next();
				if (stateB.getKey().equals(stateA.getKey()))
		...
		do {
			...
			for (Entry<String, Boolean> stateEntry : fsm.accept.entrySet()) {
				...
				Map<String, String> labelNSmap = fsm.trans.get(stateEntry
						.getKey());
				if (labelNSmap != null)
					for (Entry<String, String> labelstate : labelNSmap
							.entrySet())
						map.put(labelstate.getKey(), equivalenceClasses
								.get(labelstate.getValue()));

				newMap.put(stateEntry.getKey(), map);
				if (!sortedRows.containsKey(map)) {
					equivalenceClassNumber++;
					sortedRows.put(map, equivalenceClassNumber);
					newEquivClasses.put(stateEntry.getKey(),
							equivalenceClassNumber);
				} else
					newEquivClasses.put(stateEntry.getKey(), sortedRows
							.get(map));
			}
			...
			for (Entry<String, Integer> stateA : equivalenceClasses.entrySet()) {
				...
				while (stateB_It.hasNext()) {
					...
					if (stateA.getValue().equals(stateB.getValue())
							&& !newEquivClasses.get(stateA.getKey()).equals(
									newEquivClasses.get(stateB.getKey()))) {// the
																			// two
																			// states
																			// used
																			// to
						...
						if (Wdata.get(stateA.getKey()).containsKey(
								stateB.getKey()))
							Wsequence = Wdata.get(stateA.getKey()).get(
									stateB.getKey());// depending on the
														// ordering in the
														// matrix, either (A,B)
														// or (B,A) should be
														// defined.
						else
							Wsequence = Wdata.get(stateB.getKey()).get(
									stateA.getKey());
						assert Wsequence != null : "In states ("
								+ stateA.getKey() + "," + stateB.getKey()
								+ ") Wsequence is null";
						assert Wsequence.isEmpty() : "In states ("
								+ stateA.getKey() + "," + stateB.getKey()
								+ ") Wsequence is non-empty and contains "
								+ Wsequence;

						// the two states used to be equivalent but not any
						// more, find the different element
						String label = computeDistinguishingLabel(
								stateA.getKey(), stateB.getKey(), newMap)
								.iterator().next();
						String toA = null;
						if (fsm.trans.containsKey(stateA.getKey()))
							toA = fsm.trans.get(stateA.getKey()).get(label);
						String toB = null;
						if (fsm.trans.containsKey(stateB.getKey()))
							toB = fsm.trans.get(stateB.getKey()).get(label);
						Wsequence.add(label);
						if (toA != null && toB != null) // these can be null at
														// the first iteration,
														// where states are
														// distinguished based
														// on their response to
														// inputs rather then on
														// the states they lead
														// to.
						{
							List<String> Wprevious = null;
							if (Wdata.get(toA).containsKey(toB))
								Wprevious = Wdata.get(toA).get(toB);// depending
																	// on the
																	// ordering
																	// in the
																	// matrix,
							...
							assert Wprevious != null : "In states ("
									+ stateA.getKey() + "," + stateB.getKey()
									+ ") previous pair (" + toA + "," + toB
									+ ") has a null sequence";
							assert !Wprevious.isEmpty() : "In states ("
									+ stateA.getKey() + "," + stateB.getKey()
									+ ") previous pair (" + toA + "," + toB
									+ ") was not separated";
			...
			equivalenceClasses = newEquivClasses;
			newEquivClasses = new LinkedHashMap<String, Integer>();
		...
		if (oldEquivalenceClassNumber == fsm.accept.entrySet().size()) {
			for (Entry<String, Integer> stateA : equivalenceClasses.entrySet()) {
				...
				while (stateB_It.hasNext()) {
					...
					if (stateB.getKey().equals(stateA.getKey()))
						break; // we only process a triangular subset.
					List<String> seq = Wdata.get(stateA.getKey()).get(
							stateB.getKey());
					if (!seq.isEmpty())
						result.add(seq);
				}
			}
		} else {// report equivalent states
			...
			for (Entry<String, Integer> stateA : equivalenceClasses.entrySet()) {
				...
				while (stateB_It.hasNext()) {
					...
					if (stateB.getKey().equals(stateA.getKey()))
						break; // we only process a triangular subset.
					if (stateA.getValue().equals(stateB.getValue())
							&& !stateA.getKey().equals(stateB.getKey())) {
						equivalentStates.add(stateA.getKey());
						equivalentStates.add(stateB.getKey());
					}
				}
			}
			assert equivalentStates.size() > 0 : "equivalent states were not found";
			Iterator<String> equivIt = equivalentStates.iterator();
			throw new EquivalentStatesException(equivIt.next(), equivIt.next(),
					equivalentStates.size());
		}
		return result;
	}

	/**
	 * Given a map from pairs of states to sets of labels which distinguish
	 * between these pairs, this method returns a label which will distinguish
	 * the largest number of pairs.
	 * 
	 * @param distinguishingLabels
	 *            a map from pairs of states to sets of distinguishing labels.
	 * @return
	 */
	public static String computeTopLabel(
			Map<String, Map<String, Set<String>>> distinguishingLabels) {
		Map<String, AtomicInteger> distLabelUsage = new HashMap<String, AtomicInteger>();// reset
																							// the
																							// histogram
																							// ...
		String topLabel = null;
		int topLabelCounter = -1;// ... and the top element
		for (Entry<String, Map<String, Set<String>>> stateAdist : distinguishingLabels
				.entrySet()) {
			for (Entry<String, Set<String>> stateB : stateAdist.getValue()
					.entrySet()) {// the two states used to be in the same
									// equivalence class, now they are in
									// different ones, hence we populate the
									// matrix.
				for (String label : stateB.getValue()) {
					...
					if (counter.get() > topLabelCounter) {
						topLabel = label;
						topLabelCounter = counter.get();// record which label is
														// the top now
					}
				}
			}
	 ...
	 * Computes a characterising set, assuming that there are no unreachable
	 * states (with unreachable states, it will take a bit longer to perform the
	 * computation). Additionally, it attempts to reduce the size of W.
	 * 
	 * @param fsm
	 *            the machine to process
	 * @param alphabet
	 * @return characterising set
	 */
	public static List<List<String>> computeWSet(FSMStructure fsm)
			throws EquivalentStatesException {
		...
		do {
			...
			distinguishingLabels.clear();// clear a map from pairs of states
											// to sets of labels which
			...
			for (Entry<String, Integer> stateA : equivalenceClasses.entrySet()) {
				...
				while (stateB_It.hasNext()) {
					...
					if (stateA.getValue().equals(stateB.getValue())
						...
						// the two states used to be equivalent but not any
						// more, find inputs which
						// distinguish between them and update the histogram to
						// count the number
						// of inputs which can be used distuigish between states
						// at this stage.
						Map<String, Set<String>> stateToDist = distinguishingLabels
								.get(stateA.getKey());
						if (stateToDist == null)
							stateToDist = new HashMap<String, Set<String>>();
						stateToDist.put(stateB.getKey(),
								computeDistinguishingLabel(stateA.getKey(),
										stateB.getKey(), newMap));
			...
			while (topLabel != null) {
				...
				// Now topLabel is the most often used one, we use it to
				// separate all relevant states
				for (Entry<String, Map<String, Set<String>>> stateA : distinguishingLabels
						.entrySet()) {
					for (Entry<String, Set<String>> stateB : stateA.getValue()
							.entrySet()) {// the two states used to be in the
											// same equivalence class, now they
											// are in different ones, hence we
											// populate the matrix.

						if (distinguishingLabels.get(stateA.getKey()).get(
								stateB.getKey()).contains(topLabel)) {
							List<String> Wsequence = null;
							if (Wdata.get(stateA.getKey()).containsKey(
									stateB.getKey()))
								Wsequence = Wdata.get(stateA.getKey()).get(
							...
							String toA = null;
							if (fsm.trans.containsKey(stateA.getKey()))
								toA = fsm.trans.get(stateA.getKey()).get(
										topLabel);
							String toB = null;
							if (fsm.trans.containsKey(stateB.getKey()))
								toB = fsm.trans.get(stateB.getKey()).get(
										topLabel);
							Wsequence.add(topLabel);
							if (toA != null && toB != null) // these can be null
															// at the first
															// iteration, where
															// states are
															// distinguished
							...
							// now that we added the top label, we need to
							// remove it from the appropriate sets of labels.
							distinguishingLabels.get(stateA.getKey()).get(
									stateB.getKey()).clear();
						}
					}
				}
