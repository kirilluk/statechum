...
public class TestFSMAlgo {
	...
	/**
	 * Given a textual representation of an fsm, builds a corresponding Jung
	 * graph
	 * 
	 * @param fsm
	 *            the textual representation of an FSM
	 * @param name
	 *            graph name, to be displayed as the caption of the Jung window.
	 * @return Jung graph for it
	 * @throws IllegalArgumentException
	 *             if fsm cannot be parsed.
	 */
	public static DirectedSparseGraph buildGraph(String fsm, String name) {
		...
		new TestFSMParser.fsmParser(fsm)
				.parse(new TestFSMParser.TransitionReceiver() {
					public void put(String from, String to, String label,
							boolean accept) {
						CmpVertex fromVertex = existingVertices.get(from), toVertex = existingVertices
								.get(to);

						if (fromVertex == null) {
							fromVertex = new DeterministicDirectedSparseGraph.DeterministicVertex();
							if (existingVertices.isEmpty())
								fromVertex.addUserDatum(JUConstants.INITIAL,
										true, UserData.SHARED);
							fromVertex.addUserDatum(JUConstants.ACCEPTED, true,
									UserData.SHARED);
							fromVertex.addUserDatum(JUConstants.LABEL, from,
									UserData.SHARED);
							existingVertices.put(from, fromVertex);
							g.addVertex(fromVertex);
						} else if (!Boolean.valueOf(fromVertex.getUserDatum(
								JUConstants.ACCEPTED).toString()))
							throw new IllegalArgumentException(
									"conflicting acceptance assignment on vertex "
											+ from);
						...
						if (from.equals(to)) {
							...
							toVertex.removeUserDatum(JUConstants.ACCEPTED); // in
																			// case
																			// we've
																			// got
																			// a
																			...
																			// same
																			// state
							toVertex.addUserDatum(JUConstants.ACCEPTED, accept,
									UserData.SHARED);
							toVertex.addUserDatum(JUConstants.LABEL, to,
									UserData.SHARED);
							existingVertices.put(to, toVertex);
							g.addVertex(toVertex);
						} else if (RPNIBlueFringeLearner.isAccept(toVertex) != accept)
							throw new IllegalArgumentException(
									"conflicting acceptance assignment on vertex "
											+ to);
						...
						if (edge == null) {
							edge = new DeterministicDirectedSparseGraph.DeterministicEdge(
									fromVertex, toVertex);
							edge.addUserDatum(JUConstants.LABEL,
									new HashSet<String>(), UserData.CLONE);
							g.addEdge(edge);
							existingEdges.put(pair, edge);
						}

						Set<String> labels = (Set<String>) edge
	...
	public static class FSMStructure {
		/**
		 * The transition transition diagram, in which every state is mapped to
		 * a map between an input (label) and a target state.
		 */
		public final Map<String, Map<String, String>> trans;

		/**
		 * All states of the machine should be in the domain of this function;
		 * for a given state, this function will return
		 * 
		 * <pre>
		 * true
		 * </pre>
		 * 
		 * if it is an accept state and
		 * 
		 * <pre>
		 * false
		 * </pre>
		...
		public FSMStructure(Map<String, Map<String, String>> transitions,
				Map<String, Boolean> a, String initState) {
			trans = transitions;
			accept = a;
			init = initState;
		}
		...
		public FSMStructure() {
			trans = new TreeMap<String, Map<String, String>>();
			accept = new TreeMap<String, Boolean>();
		}
		...
		public boolean equals(Object o) {
			...
			return accept.equals(otherStruct.accept)
	...
	public final void testFSMStructureEquals2() {
		FSMStructure a = getGraphData(buildGraph("A-a->A-b->B",
				"testFSMStructureEquals2"));
		FSMStructure b = getGraphData(buildGraph("A-a->A-b->B",
				"testFSMStructureEquals2"));
		Assert.assertTrue(a.equals(b));
	}
	...
	{
		assertTrue(createLabelToStateMap(new LinkedList<String>(), "junk", null)
				.isEmpty());
		Map<String, String> map = new HashMap<String, String>();
		assertSame(map, createLabelToStateMap(new LinkedList<String>(), "junk",
				map));
	...
	public void testCreateLabelToStateMap2() // test for no changes
	{
		...
		expected.putAll(trans);
		assertSame(trans, WMethod.createLabelToStateMap(
				new LinkedList<String>(), "junk", trans));
		assertTrue(expected.equals(trans));
	}

	@Test
	public void testCreateLabelToStateMap3() // test for correct data being
												// added
	{
		Map<String, String> trans = new HashMap<String, String>();
		trans.put("a", "A");
		...
		expected.put("e", "A");
		expected.put("g", "A");
		assertSame(trans, createLabelToStateMap(Arrays.asList(new String[] {
				"g", "e" }), "A", trans));
		assertTrue(expected.equals(trans));
	}

	@Test
	public void testCreateLabelToStateMap4() // test for correct data being
												// added
	{
		Map<String, String> trans = new HashMap<String, String>();
		trans.put("a", "A");
	...
	@Test
	public void testCreateLabelToStateMap5() // test for correct data being
												// added
	{
		Map<String, String> trans = new HashMap<String, String>();
		trans.put("a", "A");
	...
	@Test
	public void testCreateLabelToStateMap6() // test for correct data being
												// added when an empty
	...
	@Test
	public void testCreateLabelToStateMap7() // test for correct data being
												// added when null is passed
	{
		Map<String, String> expected = new HashMap<String, String>();
		expected.put("e", "A");
		expected.put("b", "A");
		assertTrue(expected.equals(createLabelToStateMap(Arrays
				.asList(new String[] { "b", "e" }), "A", null)));
	}

	@Test
	public void testCreateLabelToStateMap8() // test for correct detection of
												// nondeterminism
	{
		Map<String, String> trans = new HashMap<String, String>();
		trans.put("a", "A");
		...
		try {
			createLabelToStateMap(Arrays.asList(new String[] { "b", "e" }),
					"A", trans);
		} catch (IllegalArgumentException e) {
			assertTrue("incorrect exception thrown", e.getMessage().contains(
					"nondeterminism"));
	...
	@Test
	public void testCreateLabelToStateMap9() // test for correct detection of
												// nondeterminism
	{
		boolean exceptionThrown = false;
		try {
			createLabelToStateMap(Arrays.asList(new String[] { "b", "b" }),
					"A", null);
		} catch (IllegalArgumentException e) {
			assertTrue("incorrect exception thrown", e.getMessage().contains(
					"nondeterminism"));
	...
	public void testGraphConstruction1() {
		...
		DirectedSparseGraph g = buildGraph("A--a-->B-b->C-c->A",
				"testConstruction1");
		FSMStructure graph = getGraphData(g);
		expected.trans.put("A", createLabelToStateMap(Arrays
				.asList(new String[] { "a" }), "B", null));
		expected.trans.put("B", createLabelToStateMap(Arrays
				.asList(new String[] { "b" }), "C", null));
		expected.trans.put("C", createLabelToStateMap(Arrays
				.asList(new String[] { "c" }), "A", null));
		expected.accept.put("A", true);
		expected.accept.put("B", true);
		expected.accept.put("C", true);

		assertEquals("A", graph.init);
		assertEquals("incorrect vertice set", true, expected.accept
				.equals(graph.accept));
		assertEquals("incorrect transition set", true, graph.trans
				.equals(expected.trans));
	}

	@Test
	public void testGraphConstruction2() {
		FSMStructure expected = new FSMStructure();
		DirectedSparseGraph g = buildGraph("A--a-->B-b->C-c->A-b->B-a-#D",
				"testConstruction2");
		g.setUserDatum(JUConstants.TITLE, "testConstruction2", UserData.SHARED);
		FSMStructure graph = getGraphData(g);
		expected.trans.put("A", createLabelToStateMap(Arrays
				.asList(new String[] { "a", "b" }), "B", null));
		expected.trans.put("B", createLabelToStateMap(Arrays
				.asList(new String[] { "b" }), "C", createLabelToStateMap(
				Arrays.asList(new String[] { "a" }), "D", null)));
		expected.trans.put("C", createLabelToStateMap(Arrays
				.asList(new String[] { "c" }), "A", null));
		expected.trans.put("D", createLabelToStateMap(Collections.EMPTY_LIST,
				null, null));
		expected.accept.put("A", true);
		expected.accept.put("B", true);
		expected.accept.put("C", true);
		...
		assertEquals("incorrect vertice set", true, expected.accept
				.equals(graph.accept));
		assertEquals("incorrect transition set", true, expected.trans
				.equals(graph.trans));
	}
	...
	public void testGraphConstruction3() {
		FSMStructure expected = new FSMStructure();
		DirectedSparseGraph g = buildGraph(
				"A--a-->B<-b--C-c->A-b->A-c->A\nB-d->B-p->C\nB-q->C\nB-r->C\n",
				"testConstruction3");
		FSMStructure graph = getGraphData(g);
		expected.trans.put("A", createLabelToStateMap(Arrays
				.asList(new String[] { "b", "c" }), "A", createLabelToStateMap(
				Arrays.asList(new String[] { "a" }), "B", null)));
		expected.trans.put("B", createLabelToStateMap(Arrays
				.asList(new String[] { "d" }), "B", createLabelToStateMap(
				Arrays.asList(new String[] { "r", "p", "q" }), "C", null)));
		expected.trans.put("C", createLabelToStateMap(Arrays
				.asList(new String[] { "b" }), "B", createLabelToStateMap(
				Arrays.asList(new String[] { "c" }), "A", null)));
		expected.accept.put("A", true);
		expected.accept.put("B", true);
		expected.accept.put("C", true);
	...
	public void testGraphConstruction4() {
		...
		expected.trans.put("B", createLabelToStateMap(Arrays
				.asList(new String[] { "d" }), "B", createLabelToStateMap(
				Arrays.asList(new String[] { "r", "p", "q" }), "C", null)));
		expected.trans.put("D", createLabelToStateMap(Arrays
				.asList(new String[] { "b" }), "B", createLabelToStateMap(
				Arrays.asList(new String[] { "c" }), "A", null)));
		expected.trans.put("C", createLabelToStateMap(Collections.EMPTY_LIST,
				null, null));
		expected.accept.put("A", true);
		expected.accept.put("B", true);
	...
	public void testGraphConstruction5() {
		...
		expected.trans.put("A", createLabelToStateMap(Arrays
				...
				Arrays.asList(new String[] { "a" }), "B", null)));
		expected.trans
				.put("B", createLabelToStateMap(Arrays
						.asList(new String[] { "d" }), "B",
						createLabelToStateMap(Arrays.asList(new String[] { "b",
								"r", "p", "q" }), "C", null)));
		expected.trans.put("C", createLabelToStateMap(Collections.EMPTY_LIST,
				null, null));
		expected.accept.put("A", true);
	...
	public void testGraphConstruction6() // checks loop support
	{
		FSMStructure expected = new FSMStructure();
		DirectedSparseGraph g = buildGraph(
				"P-c->P<-b-Q_State<-a-P-b->P\nQ_State-a->Q_State",
				"testConstruction6");
		FSMStructure graph = getGraphData(g);
		expected.trans.put("P", createLabelToStateMap(Arrays
				.asList(new String[] { "b", "c" }), "P", createLabelToStateMap(
				Arrays.asList(new String[] { "a" }), "Q_State", null)));
		expected.trans.put("Q_State", createLabelToStateMap(Arrays
				.asList(new String[] { "a" }), "Q_State",
				createLabelToStateMap(Arrays.asList(new String[] { "b" }), "P",
						null)));
	...
	public void testGraphConstructionFail1a() {
		...
		try {
			buildGraph("A--a-->B<-b-CONFL\nA-b->A-c->A\nB-d->B-p-#CONFL",
					"testGraphConstructionFail1a");
		} catch (IllegalArgumentException e) {
			assertTrue("correct exception not thrown", e.getMessage().contains(
					"conflicting")
					&& e.getMessage().contains("CONFL"));
			exceptionThrown = true;
		}
	...
	public static void checkWithVertex(Vertex v,
			String expectedExceptionString, String testName) {
		final DirectedSparseGraph g = buildGraph(
				"A--a-->B<-b-CONFL\nA-b->A-c->A\nB-d->B-p->CONFL", testName);
		getGraphData(g);// without the vertex being added, everything should be
						// fine.
	...
	public void testGraphConstructionFail2() {
		...
		v.addUserDatum(JUConstants.LABEL, "B", UserData.SHARED);
	...
	public void testGraphConstructionFail3() {
		DirectedSparseVertex v = new DirectedSparseVertex();
		v.addUserDatum(JUConstants.ACCEPTED, true, UserData.SHARED);
		v.addUserDatum(JUConstants.LABEL, "CONFL", UserData.SHARED);
	...
	public void testGraphConstructionFail4() {
		DirectedSparseVertex v = new DirectedSparseVertex();
		v.addUserDatum(JUConstants.ACCEPTED, true, UserData.SHARED);
		v.addUserDatum(JUConstants.LABEL, "Q", UserData.SHARED);
	...
	public void testGraphConstructionFail5a() {
		...
		checkWithVertex(v, "invalid init property",
				"testGraphConstructionFail5a");
	}
	...
	public void testGraphConstructionFail5b() {
		...
		checkWithVertex(v, "invalid init property",
				"testGraphConstructionFail5b");
	}

	@Test
	public void testGraphConstructionFail6() // missing initial state in an
												// empty graph
	{
		boolean exceptionThrown = false;
		try {
			getGraphData(new DirectedSparseGraph());// now getGraphData should
													// choke.
		} catch (IllegalArgumentException e) {
			assertTrue("correct exception not thrown", e.getMessage().contains(
					"missing initial"));
	 ...
	 *            graph to check
	 * @param fsm
	 *            the string representation of the machine which the graph
	...
	public void testCheckEq() {
		DirectedSparseGraph g = buildGraph("P-a->Q_State-b->P-c->P",
				"testCheckEq");
		checkEq(g, "P-c->P<-b-Q_State<-a-P");
	}

	/**
	 * This one is used to indicate that a two machines are not accepting the
	 * same language - I need to check that it is the incompatibility exception
	 * thrown by the <i>checkM</i> method and not any other
	...
	public static void checkM(DirectedSparseGraph g, String fsm) {
		final FSMStructure graph = getGraphData(g);
		final DirectedSparseGraph expectedGraph = buildGraph(fsm,
				"expected graph");
		final FSMStructure expected = getGraphData(expectedGraph);
	...
	static protected void checkStatePairLess(String a, String b, String c,
			String d) {
	...
	/**
	 * Checks the equivalence between the two states, stateG of graphA and
	 * stateB of graphB. Unreachable states are ignored.
	 */
	public static void checkM(FSMStructure graph, FSMStructure expected,
			String stateGraph, String stateExpected) {
		Queue<StringPair> currentExplorationBoundary = new LinkedList<StringPair>();// FIFO
																					// queue

		Set<StringPair> statesAddedToBoundary = new HashSet<StringPair>();
		currentExplorationBoundary
				.add(new StringPair(stateGraph, stateExpected));
		statesAddedToBoundary.add(new StringPair(stateGraph, stateExpected));

		while (!currentExplorationBoundary.isEmpty()) {
			StringPair statePair = currentExplorationBoundary.remove();
			assert graph.accept.containsKey(statePair.a) : "state "
					+ statePair.a + " is not known to the first graph";
			assert expected.accept.containsKey(statePair.b) : "state "
					+ statePair.b + " is not known to the second graph";
			if (!graph.accept.get(statePair.a).equals(
					expected.accept.get(statePair.b)))
				throw new DifferentFSMException(
						"states "
								+ statePair.a
								+ " and "
								+ statePair.b
								+ " have a different acceptance labelling between the machines");

			Map<String, String> targets = graph.trans.get(statePair.a), expectedTargets = expected.trans
					.get(statePair.b);
			if (expectedTargets.size() != targets.size())// each of them is
															// equal to the
															// keyset size from
															// determinism
				throw new DifferentFSMException(
						"different number of transitions from state "
								+ statePair);
			...
			for (Entry<String, String> labelstate : targets.entrySet()) {
				...
				if (!expectedTargets.containsKey(label))
					throw new DifferentFSMException(
							"no transition with expected label " + label
									+ " from a state corresponding to "
									+ statePair.b);
	...
	public void testCheckM2() {
		checkM(buildGraph("A-a->B-b->C-d-#F#-b-A", "testCheck2"),
				"B-a->C-b->D\nB-b-#REJ\nD-d-#REJ");
	}
	...
	public void testCheckM3() {
		...
		checkM(buildGraph(another.replace('A', 'Q').replace('B', 'G').replace(
				'C', 'A'), "testCheck3"), expected);
	}
	...
	public void testCheckM4() // multiple reject states
	{
		...
		checkM(buildGraph(another.replace('A', 'Q').replace('B', 'G').replace(
				'C', 'A'), "testCheck4"), expected);
	}

	@Test
	public void testCheckM5() {
		checkM(buildGraph("A-a->B-b->B-a->C", "testCheck5"),
				"S-a->U<-b-U\nQ<-a-U");
	}

	@Test
	public void testCheckM6() {
		final FSMStructure graph = getGraphData(buildGraph("A-a->B-b->B-a->C",
				"testCheck6"));
		final FSMStructure expected = getGraphData(buildGraph(
	...
	{
		final FSMStructure graph = getGraphData(buildGraph(
				"S-a->A\nS-b->B\nS-c->C\nS-d->D\nS-e->E\nS-f->F\nS-h->H-d->H\nA-a->A1-b->A2-a->K1-a->K1\nB-a->B1-b->B2-b->K1\nC-a->C1-b->C2-a->K2-b->K2\nD-a->D1-b->D2-b->K2\nE-a->E1-b->E2-a->K3-c->K3\nF-a->F1-b->F2-b->K3",
				"testCheckM_multipleEq1"));
	...
	{
		final DirectedSparseGraph g = buildGraph(
				"S-a->A-a->D-a->D-b->A-b->B-a->D\nB-b->C-a->D\nC-b->D\nS-b->N-a->N-b->N",
				"testCheckM_multipleEq2");
		final FSMStructure graph = getGraphData(g);
		List<String> states = Arrays.asList(new String[] { "S", "A", "B", "C",
				"D", "N" });
		for (String stA : states)
			for (String stB : states)
				assertTrue("states " + stA + "and " + stB
						+ " should be equivalent", checkMBoolean(graph, graph,
						stA, stB));
	}
	...
	{
		final DirectedSparseGraph g = buildGraph("A-a->B-a->C-a->A-b->C-b->B",
				"testCheckM_multipleEq4");
		final FSMStructure graph = getGraphData(g);
		...
		for (String stA : states)
			for (String stB : states)
				if (stA.equals(stB))
					assertTrue("states " + stA + " and " + stB
							+ " should be equivalent", checkMBoolean(graph,
							graph, stA, stB));
				else
					assertFalse("states " + stA + " and " + stB
							+ " should not be equivalent", checkMBoolean(graph,
							graph, stA, stB));
	}

	/** Same as checkM, but returns a boolean false instead of an exception. */
	public static boolean checkMBoolean(FSMStructure graph,
			FSMStructure expected, String stateGraph, String stateExpected) {
		try {
	...
	public void testCheckMD7() // swapped transitions
	{
		...
		checkM(buildGraph(another.replace('A', 'Q').replace('B', 'G').replace(
				'C', 'A'), "testCheckMD7"), expected);
	}

	/**
	 * Given an FSM and a sequence of labels to follow, this one checks whether
	 * the sequence is correctly accepted or not, and if not whether it is
	 * rejected at the correct element.
	 * 
	 * @param fsmString
	 *            a description of an FSM
	 * @param path
	 *            a sequence of labels to follow
	 * @param ExpectedResult
	 *            the result to check
	 * @param The
	 *            name of the vertex which is expected to be returned by
	 *            getVertex
	 */
	public static void checkPath(String fsmString, String[] path,
			int ExpectedResult, String enteredName) {
		assert (enteredName == null) ? (ExpectedResult >= 0) : true;
		final DirectedSparseGraph g = buildGraph(fsmString, "sample FSM");
		final FSMStructure graph = getGraphData(g);
		assertEquals(ExpectedResult, WMethod.tracePath(graph, Arrays
				.asList(path)));
		Vertex expected = (enteredName == null) ? null
				: new ComputeStateScores(g).findVertex(enteredName);
		assertSame(expected, RPNIBlueFringeLearner.getVertex(g, Arrays
				.asList(path)));
	}

	/**
	 * Given an FSM and a sequence of labels to follow, this one checks whether
	 * the sequence is correctly accepted from a supplied state or not, and if
	 * not whether it is rejected at the correct element.
	 * 
	 * @param fsmString
	 *            a description of an FSM
	 * @param path
	 ...
	 *            getVertex
	 */
	public static void checkPathFrom(String fsmString, String startingState,
			String[] path, int ExpectedResult, String enteredName) {
		assert (enteredName == null) == (ExpectedResult >= 0);
		final DirectedSparseGraph g = buildGraph(fsmString, "sample FSM");
		final FSMStructure graph = getGraphData(g);
		assertEquals(ExpectedResult, WMethod.tracePath(graph, Arrays
				.asList(path), startingState));
		Vertex starting = new ComputeStateScores(g).findVertex(startingState);
		Vertex expected = (enteredName == null) ? null
				: new ComputeStateScores(g).findVertex(enteredName);
		assertSame(expected, RPNIBlueFringeLearner.getVertex(g, starting,
				Arrays.asList(path)));
	}

	@Test
	public void testTracePath() {
		checkPath("A-a->B-b->C-c->D", new String[] {},
				RPNIBlueFringeLearner.USER_ACCEPTED, "A");
	}

	@Test
	public void testTracePath1a() {
		checkPath("A-a->B-b->C-c->D", new String[] { "a" },
				RPNIBlueFringeLearner.USER_ACCEPTED, "B");
	}

	@Test
	public void testTracePath1b() {
		checkPathFrom("A-a->B-b->C-c->D", "B", new String[] { "b" },
				RPNIBlueFringeLearner.USER_ACCEPTED, "C");
	}

	@Test
	public void testTracePath2a() {
		checkPath("A-a->B-b->C-c->D", new String[] { "a", "b", "c" },
				RPNIBlueFringeLearner.USER_ACCEPTED, "D");
	}
	...
	public void testTracePath9() {
		checkPath("A-a->B-b->C-c-#D", new String[] { "a", "b", "c", "d", "e" },
				3, null);
	}

	/**
	 * Computes an alphabet of a given graph and adds transitions to a reject
	 * state from all states A and inputs a from which there is no B such that
	 * A-a->B (A-a-#REJECT) gets added. Note: such transitions are even added to
	 * reject vertices.
	 * 
	 * @param g
	 *            the graph to add transitions to
	 * @param reject
	 *            the name of the reject state, to be added to the graph.
	...
	public static boolean completeGraph(DirectedSparseGraph g, String reject) {
		...
		boolean transitionsToBeAdded = false;// whether and new transitions
												// have to be added.
		...
		Iterator<Vertex> vertexIt = (Iterator<Vertex>) g.getVertices()
				.iterator();
		while (vertexIt.hasNext() && !transitionsToBeAdded) {
			...
			while (outEdgeIt.hasNext()) {
				...
				outLabels.addAll((Set<String>) outEdge
						.getUserDatum(JUConstants.LABEL));
			}
		...
		if (transitionsToBeAdded) {
			...
			while (vertexIt.hasNext()) {
				Vertex v = vertexIt.next();
				if (v != rejectVertex) {// no transitions should start from the
										// reject vertex
					Set<String> outgoingLabels = new TreeSet<String>();
					outgoingLabels.addAll(alphabet);

					Iterator<DirectedSparseEdge> outEdgeIt = v.getOutEdges()
							.iterator();
					while (outEdgeIt.hasNext()) {
						DirectedSparseEdge outEdge = outEdgeIt.next();
						outgoingLabels.removeAll((Set<String>) outEdge
								.getUserDatum(JUConstants.LABEL));
					}
					if (!outgoingLabels.isEmpty()) {
						// add a transition
						DirectedSparseEdge edge = new DirectedSparseEdge(v,
								rejectVertex);
						edge.addUserDatum(JUConstants.LABEL, outgoingLabels,
								UserData.CLONE);
						g.addEdge(edge);
					}
				}
	...
	public void completeComputeAlphabet0() {
		Set<String> alphabet = WMethod
				.computeAlphabet(new DirectedSparseGraph());
		Assert.assertTrue(alphabet.isEmpty());
	}

	@Test
	public void completeComputeAlphabet1() {
		Set<String> alphabet = WMethod.computeAlphabet(buildGraph("A-a->A",
				"completeComputeAlphabet1"));
		Set<String> expected = new HashSet<String>();
		expected.addAll(Arrays.asList(new String[] { "a" }));
		Assert.assertTrue(alphabet.equals(expected));
		DirectedSparseGraph g = buildGraph("A-a->A", "completeGraphTest1");
		Assert.assertFalse(completeGraph(g, "REJECT"));
	}

	@Test
	public void completeComputeAlphabet2() {
		Set<String> alphabet = WMethod.computeAlphabet(buildGraph(
				"A-a->A<-b-A", "completeComputeAlphabet2"));
		Set<String> expected = new HashSet<String>();
		expected.addAll(Arrays.asList(new String[] { "a", "b" }));
		Assert.assertTrue(alphabet.equals(expected));
		DirectedSparseGraph g = buildGraph("A-a->A", "completeGraphTest1");
		Assert.assertFalse(completeGraph(g, "REJECT"));
	}
	...
	public void completeComputeAlphabet3() {
		...
		Set<String> expected = new HashSet<String>();
		expected.addAll(Arrays.asList(new String[] { "a", "b", "c", "d" }));
		Assert.assertTrue(alphabet.equals(expected));
		DirectedSparseGraph g = buildGraph("A-a->A", "completeGraphTest1");
		Assert.assertFalse(completeGraph(g, "REJECT"));
	}
	...
	public void completeGraphTest2() {
		DirectedSparseGraph g = buildGraph("A-a->B-a->A", "completeGraphTest2");
		Assert.assertFalse(completeGraph(g, "REJECT"));
		checkM(g, "A-a->A");
	}
	...
	public void completeGraphTest3() {
		DirectedSparseGraph g = buildGraph("A-a->A<-b-A", "completeGraphTest3");
		Assert.assertFalse(completeGraph(g, "REJECT"));
		checkM(g, "A-b->A-a->A");
	}
	...
	public void completeGraphTest4() {
		DirectedSparseGraph g = buildGraph("A-a->B-b->A", "completeGraphTest4");
		Assert.assertTrue(completeGraph(g, "REJECT"));
		checkM(g, "A-a->B-b->A\nA-b-#REJECT#-a-B");
	}

	@Test
	public void completeGraphTest4b() {
		DirectedSparseGraph g = buildGraph("A-a->B-b->A-b->A",
				"completeGraphTest4b");
		Assert.assertTrue(completeGraph(g, "REJECT"));
	...
	public void completeGraphTest5() {
		DirectedSparseGraph g = buildGraph("A-a->A-b->B-c->B",
				"completeGraphTest5");
		Assert.assertTrue(completeGraph(g, "REJECT"));
	...
	public void testFindVertex0() {
		RPNIBlueFringeLearner.findVertex(JUConstants.JUNKVERTEX, null,
				new DirectedSparseGraph());
	}

	@Test
	public void testFindVertex1() {
		Assert.assertNull(RPNIBlueFringeLearner.findVertex(
				JUConstants.JUNKVERTEX, "bb", new DirectedSparseGraph()));
	}
	...
	public void testFindVertex3() {
		...
		Assert.assertNull(RPNIBlueFringeLearner.findVertex(JUConstants.LABEL,
				"D", g));
	}

	@Test
	public void testFindVertex4a() {
		Vertex v = RPNIBlueFringeLearner.findVertex(JUConstants.INITIAL,
				"anything", buildGraph("A-a->A-b->B-c->B-a->C\nQ-d->S",
						"testFindVertex4a"));
		Assert.assertNull(v);
	}
	...
	public void testFindVertex4b() {
		Vertex v = RPNIBlueFringeLearner
				.findVertex(JUConstants.INITIAL, true, buildGraph(
	...
	public void testFindVertex5() {
		Vertex v = RPNIBlueFringeLearner.findVertex(JUConstants.LABEL, "A",
				buildGraph("A-a->A-b->B-c->B-a->C\nQ-d->S", "testFindVertex5"));
		Assert.assertEquals("A", v.getUserDatum(JUConstants.LABEL));
	}

	@Test
	public void testFindVertex6() {
		Vertex v = RPNIBlueFringeLearner.findVertex(JUConstants.LABEL, "C",
	...
	public void testFindVertex7() {
		Vertex v = RPNIBlueFringeLearner.findVertex(JUConstants.LABEL, "S",
	...
	public void testFindVertex8() {
		Vertex v = RPNIBlueFringeLearner.findVertex(JUConstants.LABEL, "Q",
	...
	public void testFindInitial1() {
		Vertex v = RPNIBlueFringeLearner.findInitial(buildGraph(
	...
	/**
	 * Builds a set of sequences from a two-dimensional array, where each
	 * element corresponds to a sequence.
	 * 
	 * @param data
	 *            source data
	...
	/**
	 * Builds a map from an array, where each element corresponds to a pair of a
	 * string array (representing a sequence) and a string (representing flags
	...
	public static Map<String, String> buildStringMap(Object[][] data) {
		...
		for (Object[] str : data) {
			if (str.length != 2)
				throw new IllegalArgumentException(
						"more than two elements in sequence " + str);
			if (str[0] == null || str[1] == null
					|| !(str[0] instanceof String[])
					|| !(str[1] instanceof String))
				throw new IllegalArgumentException("invalid data in array");// TODO:
																			// to
																			// test
																			// that
																			// this
																			...
																			// is
																			// thrown.
			result.put(ArrayOperations.seqToString(Arrays
					.asList((String[]) str[0])), (String) str[1]);
		}
		return result;
	}
	...
	public void testBuildSet2() {
		...
		assertTrue(expectedResult
				.equals(buildSet(new String[][] { new String[] {} })));
	}
	...
	public void testBuildSet3B() {
		...
		expectedResult.add(Arrays.asList(new String[] { "a", "b", "c" }));
		assertTrue(expectedResult
				.equals(buildSet(new String[][] { new String[] { "a", "b", "c" } })));
	}
	...
	public void testBuildSet4() {
		...
		assertTrue(expectedResult.equals(buildSet(new String[][] {
				new String[] { "a", "b", "c" }, new String[] { "h", "q", "i" },
				new String[] {}, new String[] { "g", "t" } })));
	}
	...
	public void testBuildStringMap2() {
		Map<String, String> expectedResult = new HashMap<String, String>();
		expectedResult.put("a", "value2");
		expectedResult.put("b", "value3");
	...
	public void testBuildStringMap3() {
		Map<String, String> expectedResult = new HashMap<String, String>();
		expectedResult.put("a", "value1");
		expectedResult.put("strC", "value2");
		expectedResult.put("b", "value3");
	...
	public void testBuildStringMap6() {
		...
		assertTrue(expectedResult.equals(buildStringMap(new Object[][] {
				new Object[] { new String[] { "strC" }, "value1" },
				new Object[] { new String[] { "a" }, "c", "d" },// an invalid
																// sequence -
																// too many
																// elements
				new Object[] { new String[] { "b" }, "value3" } })));
	...
	public void testBuildStringMap7() {
		...
		assertTrue(expectedResult.equals(buildStringMap(new Object[][] {
				new Object[] { new String[] { "strC" }, "value1" },
				new Object[] { new Object(), "c" },// an invalid sequence -
													// wrong type of the first
													// element
				new Object[] { new String[] { "b" }, "value3" } })));
	}
	...
	public void testBuildStringMap8() {
		...
		assertTrue(expectedResult.equals(buildStringMap(new Object[][] {
				new Object[] { new String[] { "strC" }, "value1" },
				new Object[] { "text", "c" },// an invalid sequence - wrong
	...
	public void testBuildStringMap9() {
		...
		assertTrue(expectedResult.equals(buildStringMap(new Object[][] {
				new Object[] { new String[] { "strC" }, "value1" },
				new Object[] { new String[] { "a" }, new Object() },// an
																	// invalid
																	// sequence
																	// - wrong
																	// type of
	...
	public void testBuildStringMap11() {
		...
		assertTrue(expectedResult.equals(buildStringMap(new Object[][] {
				new Object[] { new String[] { "strC" }, "value1" },
				new Object[] { null, "a" },// an invalid sequence - null in the
											// second element
	 ...
	 * 
	 * @param tTable
	 *            table, where tTable[source][input]=targetstate
	 * @param vFrom
	 *            the order in which elements from tTable are to be used.
	 * @param rejectNumber
	 *            the value of an entry in a tTable which is used to denote an
	 *            absence of a transition.
	 * @return the constructed transition structure.
	 */
	public static FSMStructure convertTableToFSMStructure(final int[][] tTable,
			final int[] vFrom, int rejectNumber) {
		if (vFrom.length == 0 || tTable.length == 0)
			throw new IllegalArgumentException("array is zero-sized");
		int alphabetSize = tTable[vFrom[0]].length;
		if (alphabetSize == 0)
			throw new IllegalArgumentException("alphabet is zero-sized");
		String stateName[] = new String[tTable.length];
		for (int i = 0; i < tTable.length; ++i)
			stateName[i] = "S" + i;
		String inputName[] = new String[alphabetSize];
		...
		for (int i = 0; i < vFrom.length; ++i) {
			...
			if (currentState == rejectNumber)
				throw new IllegalArgumentException("reject number in vFrom");
		...
		if (!statesUsed.isEmpty())
			throw new IllegalArgumentException(
					"Some states in the transition table are not included in vFrom");
		return fsm;
	}

	@Test(expected = IllegalArgumentException.class)
	public final void testConvertTableToFSMStructure1a() {
		int[][] table = new int[][] { { 4, 5, 1, 6 }, { 7, 7 } };
	...
	public final void testConvertTableToFSMStructure2() {
		int[][] table = new int[][] { { 1, 0, 1, 0 }, { 0, 1 } };
		convertTableToFSMStructure(table, new int[] { 0, 1 }, -1);
	}

	@Test(expected = IllegalArgumentException.class)
	public final void testConvertTableToFSMStructure3() {
		int[][] table = new int[][] { { 1, 0, 1, 0 }, { 0, 1, 0, 1 } };
		convertTableToFSMStructure(table, new int[] { 0, -1 }, -1);
	}

	@Test(expected = IllegalArgumentException.class)
	public final void testConvertTableToFSMStructure4a() {
		int[][] table = new int[][] { { 0, 1, -1, 2 }, { 0, 3, 0, -1 },
				{ 0, 0, 0, 6 }, { -1, -1, -1, -1 } };
		FSMStructure fsm = convertTableToFSMStructure(table, new int[] { 0, 1,
				2, 3 }, -1);
		checkM(
				fsm,
				getGraphData(buildGraph(
						"S0-i0->S0-i1->S1\nS0-i3->S2\nS1-i0->S0\nS1-i1->S3\nS1-i2->S0\nS2-i0->S0\nS2-i1->S0\nS2-i2->S0\nS2-i3->S0",
						"testConvertTableToFSMStructure4")), "S0", "S0");
	}
	...
	public final void testConvertTableToFSMStructure5() {
		int[][] table = new int[][] { { 0, 1, -1, 3 }, { 0, 3, 0, -1 },
				{ 0, 0, 0, 6 }, { -1, -1, -1, -1 } };
		FSMStructure fsm = convertTableToFSMStructure(table, new int[] { 0, 1,
				3 }, -1);
		checkM(fsm, getGraphData(buildGraph(
				"S0-i0->S0-i1->S1\nS0-i3->S2\nS1-i0->S0\nS1-i1->S3\nS1-i2->S0",
				"testConvertTableToFSMStructure4")), "S0", "S0");
	}
	...
	public final void testConvertTableToFSMStructure6() {
		int[][] table = new int[][] { { 0, 1, -1, 3 }, { 0, 3, 0, -1 },
				{ 0, 0, 0, 6 }, { -1, -1, -1, -1 } };
		FSMStructure fsm = convertTableToFSMStructure(table, new int[] { 1, 0,
				3 }, -1);
		checkM(fsm, getGraphData(buildGraph(
				"S0-i0->S0-i1->S1\nS0-i3->S2\nS1-i0->S0\nS1-i1->S3\nS1-i2->S0",
				"testConvertTableToFSMStructure4")), "S0", "S0");
	...
	public final void testConvertTableToFSMStructure7() {
		int[][] table = new int[][] { { 0, 1, -1, 3 }, { 0, 3, 0, -1 },
				{ 0, 0, 0, 6 }, { -1, -1, -1, -1 } };
		FSMStructure fsm = convertTableToFSMStructure(table, new int[] { 3, 0,
				1 }, -1);
		checkM(fsm, getGraphData(buildGraph(
				"S0-i0->S0-i1->S1\nS0-i3->S2\nS1-i0->S0\nS1-i1->S3\nS1-i2->S0",
				"testConvertTableToFSMStructure4")), "S0", "S0");
