...
public class RPNIBlueFringeLearner extends Observable {
	...
	/**
	 * Determines whether the interface pops up during test execution - the
	 * interface helps debugging but slows down execution termendously.
	 */
	protected boolean debugMode = false;

	/**
	 * Updates listeners only if this object has been modified and debug mode is
	 * on, by calling
	 * 
	 * <pre>
	 * setChanged()
	 ...
	 * 
	 * @param g
	 *            the graph to display in the associated view
	 */
	...
	/**
	 * Takes all red-labelled nodes; non-red nodes which can be reached by a
	 * single transition from any red one is labelled blue.
	 * 
	 * @param model
	...
	/**
	 * Makes a copy of the graph given to it and merges states in the pair
	 * supplied.
	 */
	protected DirectedSparseGraph mergeAndDeterminize(Graph model,
			StatePair pair) {
		Graph original = (Graph) model.copy();
		Vertex q = findVertex(JUConstants.LABEL, pair.getQ().getUserDatum(
				JUConstants.LABEL), original);
		Vertex qDash = findVertex(JUConstants.LABEL, pair.getR().getUserDatum(
				JUConstants.LABEL), original);
	...
	protected DirectedSparseGraph createAugmentedPTA(DirectedSparseGraph model,
			Collection<List<String>> sPlus, Collection<List<String>> sMinus) {
	...
	public DirectedSparseGraph learnMachine(DirectedSparseGraph model,
		...
		while (!possibleMerges.isEmpty()) {
			...
			DirectedSparseGraph temp = mergeAndDeterminize(
					(Graph) model.copy(), pair);
			if (compatible(temp, sPlus, sMinus)) {// KIRR: the should always
													// return true
				pair.getQ().setUserDatum(JUConstants.HIGHLIGHT, pair,
						UserData.SHARED);
				pair.getR().setUserDatum(JUConstants.HIGHLIGHT, pair,
						UserData.SHARED);
				setChanged();
				...
				while (questionIt.hasNext()) {
					...
					boolean accepted = isAccept(pair.getQ());// Q is the blue
																// vertex
					updateGraph(model);
					int response = checkWithEndUser(model, question,
							new Object[0]);// zero means "yes", everything else
											// is "no"
					questionCounter++;
					...
					if (response == USER_ACCEPTED) {
						...
						if (accepted == false)// KIRR: how can this be true?
						...
						if (accepted == true) {// KIRR: this cannot be false
												// either
							return learnMachine(initialise(), sPlus, sMinus);
						}
					}
	...
	protected boolean compatible(DirectedSparseGraph model,
			Collection<List<String>> sPlus, Collection<List<String>> sMinus) {
	...
	protected List<String> getShortenedQuestion(List<String> question) {
		...
		while (questionIter.hasNext()) {
			...
			if (current.equals(lastQuestion))
				...
				questionList.add(lastQuestion.concat((counter > 1) ? "(*"
						+ counter + ")" : ""));// in the string case, I could
												// add "\n" at the end
				counter = 1;
	 ...
	 * 
	 * @param a
	 *            the class holding stored answers.
	 */
		...
		try {
			SwingUtilities.invokeAndWait(new Runnable() {
				public void run() {
					...
					final JList nonrejectElements = new JList(new String[] {
							"<html><font color=gray>a",
							"<html><font color=gray>b" });
					...
					jop = new JOptionPane(new Object[] { label,
							nonrejectElements, rejectElements },
	...
	protected List<List<String>> generateQuestions(DirectedSparseGraph model,
		...
		if (!hasAcceptedNeighbours(q)) {
			...
			sp = getShortPrefix(model, r);// shortest sequence to the red
											// state
		...
		if (loopToR || redAndBlueNeighbours) { // there either exists a loop to
												// r or will do if r and b merge
			if (loopToR) {
				Edge e = findEdge(r, r);
				HashSet labels = (HashSet<String>) e
	...
	/*
	 * The conventional approach to computing suffixes presumes that the source
	 * vertex is the root of a tree. This method does not make that presumption,
	 * but simply returns the direct successors of the source vertex that are
	 * accepted.
	 */
	protected static HashSet<List<String>> getShortSuffixes(
			DirectedSparseGraph g, Vertex v) {
	 ...
	 * PTA (those with no outgoing transitions) which satisfy the requested
	 * accept/reject labelling.
	 * 
	 * @param graph
	 * @param r
	 *            the vertex to consider.
	 * @param accepted
	 *            labelling requested.
	 * @return the list of paths.
	 */
	protected Set<List<String>> getSuffixes(DirectedSparseGraph graph,
			Vertex r, boolean accepted) {
	...
	/**
	 * Given a sequence of edges, builds a set of sequences of labels which will
	 * traverse the given sequence. This builds l[0]*l[1] .. where l[i] is a set
	 * of labels associated with l[i] and * is pairwise set concatenation
	 * operation. KIRR: this is the set multiplication from test set generation
	 * algorithm - surely the code must be possible to share
	 * 
	 * @param l
	 *            path
	 * @return
	...
	protected static Set<List<String>> getPaths(List<Edge> l) {
		TreeMap<Integer, Set<List<String>>> returnSet = new TreeMap<Integer, Set<List<String>>>();// KIRR:
																									// this
																									// should
																									// not
																									// be
																									...
																									// is
																									// best
		for (int i = 0; i < l.size(); i++) {// for each element of the source
											// list
			Edge e = l.get(i);
	...
	/**
	 * Returns a sequence of names labelling a shortest path from the initial
	 * node to node q.
	 */
	protected static List<String> getShortPrefix(DirectedSparseGraph model,
			Vertex q) {
		Vertex init = findInitial(model);
		UnweightedShortestPath p = new UnweightedShortestPath(model);
		Iterator<Edge> pathIt = ShortestPathUtils.getPath(p, init, q)
				.iterator();
	...
	protected static DirectedSparseGraph merge(DirectedSparseGraph model,
			StatePair pair) {
		Vertex q = pair.getQ();
		...
		while (inEdges.hasNext()) {
			...
			DirectedSparseEdge eDash = new DirectedSparseEdge(e.getSource(),
					qDash);
			eDash.addUserDatum(JUConstants.LABEL, e
					.getUserDatum(JUConstants.LABEL), UserData.CLONE);
			if (!e.getSource().getSuccessors().contains(qDash))
				model.addEdge(eDash);
			else {
				Edge existing = findEdge(e.getSource(), qDash);
				Set<String> labels = (Set<String>) existing
						.getUserDatum(JUConstants.LABEL);// KIRR: if you use
															// UserData.SHARED,
															// you do not need
															// to copy the
															// result back using
				...
				existing
						.setUserDatum(JUConstants.LABEL, labels, UserData.CLONE);
			}
			removeEdges.add(e);
		}
		...
		while (outEdges.hasNext()) {
			...
			DirectedSparseEdge eDash = new DirectedSparseEdge(qDash, e
					.getDest());
			eDash.addUserDatum(JUConstants.LABEL, e
					.getUserDatum(JUConstants.LABEL), UserData.CLONE);
	 ...
	 * merged. This function identifies such a (B,C).
	 * 
	 * @param model
			...
			while (edgeIt.hasNext()) {
				...
				while (labelIt.hasNext()) {
					...
					if (doneLabels.get(label) == null)
						...
						if (!different(p)) // KIRR: strange - the two should
											// never be different if the
											// original pair to choose was
											// selected properly
							return p;
					}
				}
	...
	protected Stack chooseStatePairs(DirectedSparseGraph g,
		...
		TreeMap<Integer, Vector<StatePair>> scoreToPair = new TreeMap<Integer, Vector<StatePair>>();// maps
																									// scores
																									// to
																									// pairs
																									// which
		...
		while (!blueStack.isEmpty()) {
			...
			redStack
					.addAll(findVertices(JUConstants.COLOUR, JUConstants.RED, g));
			...
			if (singleSet.isEmpty()) {// no new pairs found, marking the
										// current blue vertex as red.
				blueVertex.setUserDatum(JUConstants.COLOUR, JUConstants.RED,
						UserData.SHARED);
				blueStack.clear();
				scoreToPair.clear();
				blueStack.addAll(computeBlue(g));
	...
	protected Stack createOrderedStack(TreeMap<Integer, Vector<StatePair>> sets) {
		...
		if (this.pairsMergedPerHypothesis > 0) {
			...
			for (Vector<StatePair> v : sets.values()) {
				values.addAll(v);
			}
		return values;
	}
	...
	/**
	 * Checks for shallow compatibility between states, in other words if the
	 * two are both accept or both reject states
	 * 
	 * @param pair
	 *            a pair states to check for compatibility.
	...
	/**
	 * For every pair of transitions with the same label from a supplied pair of
	 * states this function recursively invokes itself and adds 1 to the result.
	 * KIRR: this should do the W set construction to determine whether two
	 * states are equivalent, but do it in the clever way, considering
	 * incompleteness.
	 * 
	 * @param original
	...
	protected int computeScore(DirectedSparseGraph original, StatePair blueRed) {
		...
		Iterator<DirectedSparseEdge> edgesOut = blueRed.getQ().getOutEdges()
				.iterator();
	...
	protected static DirectedSparseGraph initialise() {
		...
		pta.setUserDatum(JUConstants.TITLE, "Hypothesis machine",
				UserData.SHARED);
		pta.addVertex(init);
		numberVertices(pta);
		return pta;
	...
	/**
	 * Adds a given set of sequences to a PTA, with a specific accept-reject
	 * labelling.
	 * 
	 * @param pta
	 * @param strings
	 ...
	 * @param accepted
	 *            whether sequences are accept or reject ones.
	 * @return the result of adding.
	 */
	DirectedSparseGraph augmentPTA(DirectedSparseGraph pta,
			Collection<List<String>> strings, boolean accepted) {
		Iterator<List<String>> stringsIt = strings.iterator();
		while (stringsIt.hasNext()) {
			List<String> string = stringsIt.next();
			if (string.isEmpty() && !accepted)
				throw new IllegalArgumentException(
						"since the initial state is an accept one, a negative string should not be empty");
			...
			for (int i = 1; i <= string.size(); i++) {
				...
				if (i == string.size())// KIRR: every prefix of a reject
										// sequence is an accept sequence.
					newVertex.setUserDatum(JUConstants.ACCEPTED, accepted,
							UserData.SHARED);
				else
					newVertex.setUserDatum(JUConstants.ACCEPTED, true,
							UserData.SHARED);

				if (existing == null) {
					pta.addVertex(newVertex);
					Vertex previous;
					previous = getVertex(pta, string.subList(0, i - 1));// for
																		// i==1,
																		// getVertex
																		// will
																		// return
																		...
																		// initial
																		// vertex
					DirectedSparseEdge e = new DirectedSparseEdge(previous,
							newVertex);
					Set<String> labels = new HashSet<String>();
					labels.add(string.get(i - 1));
					e.addUserDatum(JUConstants.LABEL, labels, UserData.CLONE);
					pta.addEdge(e);
				} else if (different(new StatePair(existing, newVertex))) {
					existing.addUserDatum(JUConstants.HIGHLIGHT, "whatever",
							UserData.SHARED);
					updateGraph(pta);
	...
	/**
	 * Labels vertices according to breadth-first search KIRR: this should label
	 * vertices according to their Jung ID instead, because those IDs are shown
	 * in the debugger and it is pain to dig through to find labels in
	 * user-added data.
	 * 
	 * @param pta
	 *            the graph to operate on.
	...
	protected static void numberVertices(DirectedSparseGraph pta) {
		...
		while (vertexIt.hasNext()) {
			Vertex v = vertexIt.next();
			v.removeUserDatum(JUConstants.LABEL);// since we'd like this
													// method to run multiple
													// times, once immediately
													// after initialisation and
													// subsequently when sPlus
													// and sMinus are added.
			v.addUserDatum(JUConstants.LABEL, v.toString(), UserData.SHARED);
		}
	}

	protected static Vertex getVertex(DirectedSparseGraph g, Vertex v,
			List<String> string) {
		Vertex current = v;
		if (current == null)
			return null;
		...
		for (int i = 0; i < string.size(); i++) {
			...
			DirectedSparseEdge edge = getEdgeWithLabel(current.getOutEdges(),
					label);
			if (edge == null)
				return null;
	...
	/**
	 * Finds an initial state in a graph. Returns null if the initial state was
	 * not found.
	 * 
	 * @param g
	 *            graph to search for an initial state in.
	 ...
	 *            property to search
	 * @param value
	 *            what it has to be set to, cannot be null
	 * @param g
	...
	public static Vertex findVertex(JUConstants property, Object value, Graph g) {
		if (value == null)
			throw new IllegalArgumentException(
					"value to search for cannot be null");
	...
	public static Set<Vertex> findVertices(JUConstants property, Object value,
			Graph g) {
