 	   ...
 	   final Collection<List<Label>> evaluationTestSet = LearningAlgorithms.computeEvaluationSet(referenceGraph,-1,-1);
 		...
 		final Integer frame=6;
   		//for(final Integer frame:allFrames)
   		{
 			...
 			StateMergingStatistics redReducer = LearningAlgorithms.ComputeMergeStatisticsWhenTheCorrectSolutionIsKnown.constructReducerIfUsingHVScoring(referenceGraph,LearningAlgorithms.ReferenceLearner.OverrideScoringToApply.SCORING_HV);
  			{
   				...
   				LearningAlgorithms.LearnerThatCanClassifyPairs referenceLearner = new LearningAlgorithms.LearnerThatCanClassifyPairs(initConfiguration, referenceGraph, initPTA,LearningAlgorithms.ReferenceLearner.OverrideScoringToApply.SCORING_HV,redReducer);
...

   			{
   				...
   				LearningAlgorithms.LearnerThatCanClassifyPairs referenceLearner = new LearningAlgorithms.LearnerThatCanClassifyPairs(initConfiguration, referenceGraph, ptaAfterMergingBasedOnUniques,LearningAlgorithms.ReferenceLearner.OverrideScoringToApply.SCORING_HV,redReducer);
   				referenceLearner.setPairQualityCounter(pairQualityCounter,referenceGraph,null);
 		        LearnerGraph referenceOutcome = referenceLearner.learnMachine(new LinkedList<List<Label>>(),new LinkedList<List<Label>>());
 		        //referenceOutcome.storage.writeGraphML("resources/"+name+"-ref_"+frame+".xml");
 ...
 >>>>>>> .merge-right.r793
    ...
    */
    ...
    public DifferenceToReference learnAndEstimateDifference(int ifDepth,LearnerGraph initPTA, LearnerGraph referenceGraph,Classifier c, final Collection<Label> labelsToMergeTo, final Collection<Label> labelsToMergeFrom)
    {
		...
		StateMergingStatistics redReducer = LearningAlgorithms.ComputeMergeStatisticsWhenTheCorrectSolutionIsKnown.constructReducerIfUsingHVScoring(
				referenceGraph,LearningAlgorithms.ReferenceLearner.OverrideScoringToApply.SCORING_HV);
		ReferenceLearner learner =  c != null? new PairQualityLearner.LearnerThatUsesWekaResults(parameters,learnerInitConfiguration,referenceGraph,c,initPTA, dataCollector, false):
 					new ReferenceLearner(learnerInitConfiguration,initPTA,ReferenceLearner.OverrideScoringToApply.SCORING_HV,redReducer);
