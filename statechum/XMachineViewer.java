...
public class XMachineViewer{
	...
	public XMachineViewer(XMachineModel machine, boolean vertices, boolean edges, boolean collapsed){
		this.vertices = vertices;
		this.edges = edges;
		this.collapsed = collapsed;
		if(collapsed)
			setGraph(getCollapsedGraph(new JungGraphBuilder().constructGraph(machine)));
		else
	...
	public DirectedSparseGraph getCollapsedGraph(DirectedSparseGraph g){
		...
		DirectedSparseGraph collapsed = (DirectedSparseGraph)GraphCollapser.getInstance().getCollapsedGraph(er);
	...
	private void labelClassVertices(DirectedSparseGraph g){
		...
		while(vertexIt.hasNext()){
			GraphCollapser.CollapsedSparseVertex csv = (GraphCollapser.CollapsedSparseVertex)vertexIt.next();
			HashSet rootSet = (HashSet)csv.getRootSet();
			DirectedSparseVertex dsv = (DirectedSparseVertex)rootSet.iterator().next();
	...
	private void labelClassEdges(DirectedSparseGraph g){
		...
		while(edgeIt.hasNext()){
			GraphCollapser.DirectedCollapsedEdge cse = (GraphCollapser.DirectedCollapsedEdge)edgeIt.next();
			...
			String edgeLabel = new Integer(t.getFunctions().keySet().size()).toString();
	...
	private EquivalenceRelation constructClassNameEquivalenceRelation(DirectedSparseGraph g){
		HashMap classNamesToVertices = new HashMap();
		Iterator vertexIt = g.getVertices().iterator();
		while(vertexIt.hasNext()){
			DirectedSparseVertex vertex = (DirectedSparseVertex)vertexIt.next();
	...
	public void setTransformable(){
		DefaultModalGraphMouse graphMouse = (DefaultModalGraphMouse)v.getGraphMouse();
