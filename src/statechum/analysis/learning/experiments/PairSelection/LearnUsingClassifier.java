				...
				for(final int ifDepth:new int []{1})
					for(final boolean onlyPositives:new boolean[]{true})
					{
						...
						for(final boolean useUnique:new boolean[]{false})
						{
							...
							InputStream inputStream = Files.newInputStream(Paths.get(outPathPrefix + parExperiment.getExperimentID() + ".ser"));
							...
							for(final double threshold:new double[]{3})
							{
								...
								final RBoxPlot<Long> gr_PairQuality = new RBoxPlot<>("Correct v.s. wrong", "%%", new File(outPathPrefix + "percentage_score" + selection + ".pdf"));
								final RBoxPlot<String> gr_QualityForNumberOfTraces = new RBoxPlot<>("traces", "%%", new File(outPathPrefix + "quality_traces" + selection + ".pdf"));
									...
									for(int sample=0;sample<samplesPerFSM;++sample)
										for(int trainingSample=0;trainingSample<trainingSamplesPerFSM;++trainingSample)
										{
											...
											{// first, use the learner with a classifier
												...
												final Map<Long,TrueFalseCounter> pairQualityCounter = new TreeMap<>();
												...
												{
													@Override
													public LearnerWithMandatoryMergeConstraints createLearner(LearnerEvaluationConfiguration evalCnf,LearnerGraph argReferenceGraph,WekaDataCollector argDataCollector,	LearnerGraph argInitialPTA,@SuppressWarnings("unused") ComputeMergeStatisticsWhenTheCorrectSolutionIsKnown redReducer)
													{
											...
											{// second, use a traditional learner
												...
												PairQualityLearnerRunner learnerRunner = new PairQualityLearnerRunner(dataCollector,parameters, learnerInitConfiguration)
												{
													...
													{
														return new LearningAlgorithms.ReferenceLearner(evalCnf,argInitialPTA,LearningAlgorithms.ReferenceLearner.OverrideScoringToApply.SCORING_HV,redReducer);
													}
								...
								if (experimentRunner.getPhase() == PhaseEnum.COLLECT_RESULTS || experimentRunner.getPhase() == PhaseEnum.COLLECT_AVAILABLE)
								{
									...
									DrawGraphs.spreadsheetAsDouble(
											(reference, actual) -> {
										if (reference > 0)
									...
									{
										final Map<Long,TrueFalseCounter> pairQualityCounter = new TreeMap<>();
										DrawGraphs.spreadsheetObjectsCollect(
												(key, obj) -> {
