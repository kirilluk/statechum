...
public class SlowPrefixFreeCollection extends PrefixFreeCollection {
	...
	public void addSequence(List<String> seq) {
		...
		for (List<String> s : data) {
			if (isPrefix(s, seq)) {
				assert (seqToRemove.isEmpty());// no sequences are prefixes of
												// the current one - an internal
												// consistency invariant of this
												// procedure.
				return; // the sequence to add is already a prefix of another
						// one
			}
			if (isPrefix(seq, s))
				seqToRemove.add(s);
