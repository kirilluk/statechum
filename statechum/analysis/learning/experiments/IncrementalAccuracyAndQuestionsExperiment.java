...
public class IncrementalAccuracyAndQuestionsExperiment {
	...
	public abstract static class LearnerEvaluatorGenerator {
		abstract LearnerEvaluator getLearnerEvaluator(String inputFile,
				String ouputDir, int instanceID);
	}
	...
	public abstract static class LearnerEvaluator implements Callable<String> {
		...
		protected String writeResult(OUTCOME outcome, String result) {
			...
			try {
				...
				outputWriter.write(inputFileName + FS + outcome
	...
	public static abstract class RPNIEvaluator extends LearnerEvaluator {
		...
		private void questionsExperiment(FSMStructure fsm,
				RPNIBlueFringeLearnerTestComponentOpt l, WMethod wm) {
			PosNegPrecisionRecall prNeg = computePR(fsm, l, wm,
					new HashSet<List<String>>(), sMinus);
			System.out.println(prNeg.precision + ", " + prNeg.recall);
		}

		private void posNegExperiment(FSMStructure fsm,
				RPNIBlueFringeLearnerTestComponentOpt l, WMethod wm) {
			PosNegPrecisionRecall prNeg = computePR(fsm, l, wm,
					new HashSet<List<String>>(), sMinus);
			PosNegPrecisionRecall pr = computePR(fsm, l, wm, sPlus,
					new HashSet<List<String>>());
			System.out
					.println(pr.getPosprecision() + ", " + pr.getPosrecall()/*
																			 * +",
																			 * "+pr.getNegprecision()+",
																			 * "+pr.getNegrecall()+",
																			 * "+prNeg.getPosprecision()+",
																			 * "+prNeg.getPosrecall()+",
																			 * "+prNeg.getNegprecision()+",
																			 * "+prNeg.getNegrecall()
																			 */);
		}

		private PosNegPrecisionRecall computePR(FSMStructure fsm,
				RPNIBlueFringeLearnerTestComponentOpt l, WMethod wm,
				Collection<List<String>> splus, Collection<List<String>> sminus) {

			FSMStructure learned = learn(l, splus, sminus);
			PTA_computePrecisionRecall precRec = new PTA_computePrecisionRecall(
					learned);
		...
		private FSMStructure learn(RPNIBlueFringeLearnerTestComponentOpt l,
				Collection<List<String>> sPlus, Collection<List<String>> sMinus) {
			...
			/*
			 * if(percent == 30) System.out.print(l.getQuestionCounter()+",");
			 * else System.out.println(l.getQuestionCounter());
			 */
	...
	private static void printPR(Vector<PosNegPrecisionRecall> results) {
		/*
		 * PosNegPrecisionRecall thirty = results.get(0); PosNegPrecisionRecall
		 * hundred = results.get(1);
	...
	public static Collection<List<String>> addNumberFromSamples(
			Collection<List<String>> current, Collection<List<String>> samples,
			double number) {
		double size = samples.size();
		List<String>[] sampleArray = (List<String>[]) samples
				.toArray(new List[samples.size()]);
	...
	public static final LearnerEvaluatorGenerator[] learnerGenerators = { new LearnerEvaluatorGenerator() {
		...
		LearnerEvaluator getLearnerEvaluator(String inputFile,
				String outputDir, int instanceID) {
	...
	public static void main(String[] args) {
		...
		if (args.length < 2) {
			...
			if (outputDir.canRead() || outputDir.mkdirs()) {
				experiment = new IncrementalAccuracyAndQuestionsExperiment(
						outputDir.getAbsolutePath());
				assert fileNumber * learnerGenerators.length == experiment
						.computeMaxNumber(fileNameListReader);
				for (int number = 0; number < fileNumber
						* learnerGenerators.length; ++number)
