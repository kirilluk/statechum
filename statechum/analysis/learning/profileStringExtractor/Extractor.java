...
public class Extractor {
	...
	public JTree getTree() {
		...
		while (handlerIt.hasNext()) {
			ClassMethodDefsHandler current = (ClassMethodDefsHandler) handlerIt
					.next();
			Map<Integer, Set<Integer>> classesToMethods = current
					.getClassesToMethods();
			Map<Integer, String> methodDefs = current.getMethodDefs();
			Map<Integer, String> classDefs = current.getClassDefs();
			Iterator<Integer> methodKeyIt = classesToMethods.keySet()
					.iterator();
			while (methodKeyIt.hasNext()) {
				Integer classId = methodKeyIt.next();
				String className = classDefs.get(classId).toString();
				DefaultMutableTreeNode classNameNode = getNodeForClassName(
						className, root);
	...
	private static boolean hasChildWithName(DefaultMutableTreeNode node,
			String name) {
	...
	private static DefaultMutableTreeNode getNodeForClassName(String className,
