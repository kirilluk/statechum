...
public class TranslatorParser extends DefaultHandler {
	...
	public TranslatorParser(String outputName, ClassMethodDefsHandler classMethods){
	...
	/**
	 * Findloops seems to need process IDs to start at 1, and iterate from there.
	 * For this reason we need to establish a mapping from the arbitrary TPTP class IDs
	 * to iterative consecutive process IDs.
	 * @param classId
	 * @return
	 */
	...
	private void addToFindloopsObservation(Integer classId, Integer methodId, String methodString){
		...
		stringList.add("s"+"\t"+processForSenderClass+"\t"+processForCurrentClass+"\t"+methodString+"\n");
		stringList.add("r"+"\t"+processForSenderClass+"\t"+processForCurrentClass+"\t"+methodString+"\n");
