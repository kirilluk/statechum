...
public class ConstructClassifier 
{
	...
	public static abstract class NominalAttributeClassifier extends AbstractClassifier
	{
		...
		/** Returns the position of the 'true' value in the distribution returned by {@link AbstractClassifier#distributionForInstance(Instance)}
		 */
		...
		/** Returns the position of the 'false' value in the distribution returned by {@link AbstractClassifier#distributionForInstance(Instance)}
		 */
						...
						for(int states:new int[]{20})
							for(int sample=0;sample<Math.round(samplesPerFSM*trainingDataMultiplier);++sample)
								for(int attempt=0;attempt<2;++attempt)
								{
									...
									{
										@Override
										public LearnerThatCanClassifyPairs createLearner(LearnerEvaluationConfiguration evalCnf,LearnerGraph argReferenceGraph,WekaDataCollector argDataCollector,	LearnerGraph argInitialPTA, @SuppressWarnings("unused") ComputeMergeStatisticsWhenTheCorrectSolutionIsKnown redReducer) 
										{
