...
final public class DeterministicDirectedSparseGraph {
	...
	public static DeterministicVertex copyVertex(Map<VertID,DeterministicVertex> newVertices, DirectedSparseGraph g,Vertex orig)
	{
		...
		if (newVertex == null) 
		{
			newVertex = AbstractLearnerGraph.generateNewJungVertex(origVertex);
	...
	public static void numberVertices(DirectedSparseGraph pta){
		for (DeterministicVertex v : getBFSList(pta)) {
	...
	public static List<DeterministicVertex> getBFSList(Graph g){
		List<DeterministicVertex> queue = new LinkedList<>();
		DeterministicVertex init = DeterministicDirectedSparseGraph.findInitial(g);
		queue.add(0,init);
		int i=0;
		int j= queue.size();
		Set<DeterministicVertex> done = new HashSet<>();
		while(i<j){
			DeterministicVertex v = queue.get(i);
			done.add(v);
			//noinspection unchecked
			for (DeterministicVertex succ : (Iterable<DeterministicVertex>) v.getSuccessors()) {
	...
	public static Set<DeterministicVertex> findVertices(JUConstants property, Object value, Graph g){
		Set<DeterministicVertex> vertices = new HashSet<>();
		//noinspection unchecked
		for (DeterministicVertex v : (Iterable<DeterministicVertex>) g.getVertices()) {
			if (v.getUserDatum(property) == null)
				continue;
