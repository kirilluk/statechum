...
public class RPNIBlueFringeLearnerTestComponent extends RPNIBlueFringeLearner {
	...
	public DirectedSparseGraph learnMachine(DirectedSparseGraph model,
		...
		this.sPlus = sPlus;
		this.sMinus = sMinus;
		model = createAugmentedPTA(model, sPlus, sMinus);// KIRR: node
															// labelling is done
															// by
															// createAugmentedPTA
		findInitial(model).setUserDatum(JUConstants.COLOUR, JUConstants.RED,
				UserData.SHARED);
		setChanged();
		...
		while (!possibleMerges.isEmpty()) {
			...
			pair.getR().setUserDatum(JUConstants.HIGHLIGHT, pair,
					UserData.SHARED);// since this copy of the graph will
										// really not be used, changes to it are
			...
			if (score < this.certaintyThreshold
					&& score > minCertaintyThreshold) {
				questions = generateQuestions(model, temp, pair);
				// questions = trimSet(questions); // KIRR: unnecessary by
			...
			while (questionIt.hasNext()) {
				...
				int answer = checkWithEndUser(model, question,
						new Object[] { "Test" });
				...
				if (answer == USER_ACCEPTED) {
					...
					// sMinus.add(question.subList(0, answer+1)); // KIRR:
					// without a `proper' collection in the set, I cannot
					// serialise the sets into XML

					// System.out.println(setByAuto+question.toString()+ " <no>
					// at position "+answer+", element "+question.get(answer));
					if ((answer == question.size() - 1)
							&& !isAccept(tempVertex)) {
						continue;
					} else {
			...
			if (restartLearning) {// restart learning
				model = createAugmentedPTA(initialise(), sPlus, sMinus);// KIRR:
																		// node
																		// labelling
																		// is
																		// done
																		// by
																		// createAugmentedPTA
				findInitial(model).setUserDatum(JUConstants.COLOUR,
	...
	public static Vertex getTempRed(DirectedSparseGraph model, Vertex r,
			DirectedSparseGraph temp) {
		...
		if (!pathToRed.isEmpty()) {
			...
			List<String> prefixString = (List<String>) pathToRedStrings
					.toArray()[0];
	...
	protected List<List<String>> generateQuestions(DirectedSparseGraph model,
		...
		if (loopEdge != null) {
			Collection<String> looplabels = (Collection<String>) loopEdge
					.getUserDatum(JUConstants.LABEL);
			questions.addAll(mergePrefixWithSuffixes(prefixes, looplabels,
					suffixes));
		}

		DirectedSparseGraph questionPrefixes = augmentPTA(initialise(),
				questions, true);
		...
		while (questionIt.hasNext()) {
			...
			if (pathsToPoint.isEmpty())
				continue;
			List<String> pathToPoint = (List<String>) getPaths(edgePath)
					.toArray()[0];
	...
	public static List<List<String>> mergePrefixWithSuffixes(
			Set<List<String>> sp, Collection<List<String>> suffixes) {
	...
	public static List<List<String>> mergePrefixWithSuffixes(
			Collection<List<String>> sp, Collection<String> loopLabels,
			Collection<List<String>> suffixes) {
		ArrayList<List<String>> questions = new ArrayList<List<String>>();
		Object[] prefixArray = null;
		int iterations = sp.size();
		if (sp.isEmpty()) {
	...
	public static Set<List<String>> computeSuffixes(Vertex v,
			DirectedSparseGraph model) {
	...
	private boolean containsSubString(Collection<List<String>> sPlus,
